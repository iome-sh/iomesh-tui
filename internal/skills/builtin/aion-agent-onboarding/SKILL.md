...
description: Residual-honest TUI agent ↔ aion CP/MCP onboarding (list/plan connectors → portal HITL · memory dual_write OFF · not Memory GA · never invent Connected)
---
...
**System note (s1363):** when MCP is attached (`AttachMCP`), runtime injects residual-honest `<aion-onboarding>` (`AionAgentOnboardingGuidanceNote`) with the same locks. Skill + note stay consistent; skill is the full playbook. Operator slash: `/onboard` (aliases `/aion-onboard` `/agent-onboard`).

## Workflow

1. **Point IOMESH/MCP at aion tools** — ensure aion MCP (connector + memory servers as configured) is attached.
   - Offline / missing tools → residual-honest **fail-open**; **never invent** tool green or Connected.

2. **Discover** — call MCP `list_connector_catalog` (aion v178).
...
3. **Plan** — call MCP `plan_connector_setup` with `connector_id`.
...
4. **Org installs residual snapshot** — call MCP `list_org_connector_installs` with `org_id` when present (aion v179 residual).
...
6. **Memory residual** — dual_write **OFF** · **local-primary** · **not Memory GA**.
...
7. **Operator pulse** — slash `/integrations status` · `/onboard checklist` · portal HITL.
...
| not Memory GA | Onboarding ≠ invent Memory Palace / graph RAG product green |
...
| portal HITL | Human finishes OAuth / install in console session |
...
- Builtin skill always available when skills enabled (**s1363** · molds s1251 connector + s1288 memory-advanced + s1341 gtm-draft-only).
- System note inject on `AttachMCP`: `<aion-onboarding>` via `AionAgentOnboardingGuidanceNote` (s1363).
...
- Slash residual honesty: `/onboard [help|checklist]` · `/integrations list|plan|status|signing` · `/memory status`.
