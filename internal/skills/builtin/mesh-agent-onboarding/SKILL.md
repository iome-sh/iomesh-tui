...
description: Residual-honest I/O Mesh TTFH onboard (`/onboard next ttfh` one walk R0–R4 · R1 --live ≠ R3 overlay PULSE · dual_write OFF · catalog ≠ Connected · Cloud Memory GA · never invent Connected · portal HITL when connect)
---

# Mesh agent onboarding (residual-honest)

Builtin playbook for **I/O Mesh time-to-first-heartbeat** — residual-honest path only. Molds `connector-integrations-setup` + operator onboarding checklist. **Not** install APPLY, **Cloud Memory GA**, **Not** Agent Plugins GA, **Not** dual_write ON.

**Start here (TTFH):** LLM key or Ollama · `/setup init local-memory` · attach `iomesh-memory-mcp` (Cloud Memory GA) · **R0** `iomesh ttfh --unit` (offline · no mesh) · **R1** optional `iomesh ttfh --live` (fail-open · EMPTY unless decoded · not overlay PULSE). **Optional mesh:** `IOMESH_ENDPOINT` for R1 only. Overlay `/dashboard` consume is R3, not this. **R2** `/memory ingest` ×3 (`source_hint=private`) · `/memory digest --require-sources mesh,private` (cite-both **or miss**; miss is success) · named miss: `no_mesh_pulse` · `no_private_overlay` · `conflict` · `insufficient_signal` · `linked_pr_miss` (SRE private eval · not MTTR) · `public_vs_internal` (only if public page) · `no_memo` · `crm_only_restatement` · `/memory patterns` (Beta) · `/memory facts-as-of` · miss ACK `/dashboard ack` (local ritual · no send/pay/ship). **R3** `/dashboard` consume (overlay PULSE · parked · required for E-G1 · **empty until consume** · **CLIENT ≠ PULSE**). **R4** `iomesh memory pull` after PULSE · dual_write **OFF** · pull ≠ Connected. Map: `/onboard next ttfh` (aliases `time-to-first-heartbeat`|`cite-both`|`rollout`). dual_write **OFF** · catalog ≠ Connected · Cloud Memory GA · never invent Connected.

**Cloud Memory palace bind (Gap / Partial · US-CM-JOURNEY-05):** Console entitlement is the primary attach (workspace-as-principal). Entitlement ≠ live bind. **One write path — not mirrored to a second store.** Local and Cloud Memory stay on separate paths. Cloud Memory GA is optional beside TTFH. TTFH/heartbeat is the system of record. Empty until consume. Catalog ≠ Connected. **B5** TUI host bind and **C4** SDK palace URL bind stay Gap until QA evidence. Do not invent a Connected host URL. Not an Exists Connected bind. This TUI does not ship the SDK bind. Multi-human palace read/write stays Gap.
...
2. **Settings → Agent/MCP → copy TUI fragment**
   ...
   - Connection copy is **handoff only**, not install green.

3. **Test invoke = stub|live** (42ms/no preview = stub · ≠ live tools/call · ≠ consume · Cloud Memory GA)
   - Portal test invoke is a residual-honest **probe**.
   - **Never invent** tool green or Connected · Cloud Memory GA from a probe.
...
6. **Discover** — call MCP `list_connector_catalog` (mesh v178).
...
8. **Org installs residual snapshot** — call MCP `list_org_connector_installs` with `org_id` when present (mesh v179 residual).
...
10. **Memory residual** — dual_write **OFF** · **local-primary**.
...
- **Sales/buyer claims board (s1437):** `/onboard next sales` (aliases `claims` / `buyer` / `claim-matrix` / `sales-claims` / `buyer-claims`) → `MeshAgentOnboardingNextSalesClaims` — may claim / must not claim residual-honest · three-planes grounded · **never invent Connected · Cloud Memory GA / dual-auth live**.
...
- **Marketing demo path (s1590):** `/onboard next marketing-demo` (aliases `marketing` / `sales-demo` / `demo-script` / `gtm-demo`) → `MeshAgentOnboardingNextMarketingDemoLane` — plain-language operator script for videos/sales · local agent + local memory · dual_write **OFF** · mesh optional · **never invent Connected · Cloud Memory GA** · free eng **s1590** · internal note removed from this public repository · **do not steal** bare `demo` / `sales` / `gtm`.
- **Operator readiness matrix (s1447):** `/onboard next operator` (aliases `operator-matrix` / `ops-matrix` / `operator-readiness` / `ops-readiness` / `matrix`) → `MeshAgentOnboardingNextOperatorMatrix` — consolidate demo · sales · planes · human-gates · dual-auth candidacy · policy locks residual-honest · **never invent Connected / GA / dual-auth live**.
- **setup lifecycle map (s1542+s1558 · stage 4 of edge-user-journey · P1–P7 closeout residual):** `/onboard next setup` (aliases `setup-lifecycle` / `lifecycle` / `setup_lifecycle`) → `MeshAgentOnboardingNextSetupLane` — init → preflight → reload → portal HITL → pull → analyze → drift → repair plan/apply --yes · **setup_not_probed** · dual_write OFF · package wire ≠ Connected · **repair apply ≠ invent Connected** · dual_write never auto ON · **E10 Open** · offline static ≠ live dogfood · Cloud Memory GA · setup closeout ≠ invent Connected · full first-run map: `/onboard next journey`.
- **edge-user-journey first-run map (s1558 Wave B · 7 stages):** `/onboard next journey` (aliases `edge-journey` / `user-journey` / `first-run` / `edge_user_journey`) → `MeshAgentOnboardingNextJourneyLane` — Signup → Download TUI → TUI auth/keys → Setup wizard → Connectors → Local store → Analyze · dual_write OFF · Cloud Memory GA · residual PASS ≠ invent Connected · portal HITL · no invent TUI portal SSO · host not auto · free eng **s1558** · free-floor peer **s1560+** mention only · docs `edge-user-journey.md` · `setup-lifecycle.md` · `memory-edge-usage-demo.md` · stage 6 companion `/onboard next e4` (s1566).
- **E4 client-attach stage-6 board + soft dogfood (s1566):** `/onboard next e4` (aliases `e4-dogfood` / `client-attach` / `edge-memory-e4` / `e4_attach`) → `MeshAgentOnboardingNextE4Lane` · soft `/onboard next e4 dogfood` (aliases `soft` / `samples` / `offline` / `e4-soft`) → `RunE4SoftDogfood` — journey stage 6 local store / MCP attach · tools=6 · `iomesh mcp --connect` residual · dual_write OFF · Cloud Memory GA · residual PASS ≠ invent Connected · E10 Open · tip ≠ invent forever-green product dogfood · free eng **s1566** · free-floor peer **s1568+** mention only · companion deeper tool-call `/onboard next tool-call` (s1578).
- **deeper tool-call residual + soft dogfood (s1578):** `/onboard next tool-call` (aliases `tool-calls` / `deeper-e4` / `e4-tools` / `ingest-retrieve` / `tool_call`) → `MeshAgentOnboardingNextToolCallLane` · soft `/onboard next tool-call dogfood` (aliases `soft` / `samples` / `offline` / `tool-call-soft`) → `RunDeeperToolCallSoftDogfood` — journey stage 6/7 depth after E4 attach · operator map ingest→retrieve→list→as-of · tools `memory_ingest_turn` · `memory_retrieve` · `memory_search_semantic` · `memory_list` · `memory_compact_status` · `memory_facts_as_of` · Partial→client-attach-evidence · session labels `tool_call_soft_not_run` · `soft_offline_tool_call_session_pass` · `soft_offline_tool_call_session_fail` · dual_write OFF · Cloud Memory GA · residual PASS ≠ invent Connected · E10 Open · tip ≠ invent forever-green product dogfood · free eng **s1578** · free-floor peer **s1580+** mention only · companion E10 Open reaffirm `/onboard next e10` (s1586).
- **E10 Open reaffirm residual-check (s1586):** `/onboard next e10` (aliases `e10-open` / `edge-memory-e10` / `ga-signoff` / `e10_open`) → `MeshAgentOnboardingNextE10Lane` · soft residual-check `/onboard next e10 dogfood` (aliases `soft` / `samples` / `offline` / `e10-soft` / `residual-check`) → `RunE10OpenSoftDogfood` — pin **E10 Open** after OSS packaging continuum · residual PASS ≠ invent E10 closed · residual PASS ≠ invent Connected · Cloud Memory GA · E10 stays Open · Cloud Memory GA ≠ close E10 · PASS ≠ live APPLY · session labels `e10_soft_not_run` · `soft_offline_e10_session_pass` · `soft_offline_e10_session_fail` · dual_write OFF · book-demo OFF · residual-check · free eng **s1586** · free-floor peer **s1588+** mention only · companion e4 · human-gates · OSS packaging / MIT harness ≠ control plane.
- **first-run wizard residual + soft dogfood (s1570 Wave C):** `/onboard next wizard` (aliases `first-run-wizard` / `guided` / `wave-c` / `wave_c` / `wizard-residual`) → `MeshAgentOnboardingNextWizardLane` · soft `/onboard next wizard dogfood` (aliases `soft` / `samples` / `offline` / `wizard-soft`) → `RunFirstRunWizardSoftDogfood` — deeper guided residual map after Wave B journey · session labels `wizard_soft_not_run` · `soft_offline_wizard_session_pass` · `soft_offline_wizard_session_fail` · NOT invent full interactive auto wizard · dual_write OFF · Cloud Memory GA · residual PASS ≠ invent Connected · E10 Open · portal HITL when connect · agent MCP cannot write installs · catalog ≠ Connected · no invent TUI portal SSO · host not auto · free eng **s1570** · free-floor peer **s1572+** mention only · companion still-human soft `/onboard next human-gates dogfood` (s1574).
...
- **Agentic dual-auth candidacy (s1427):** `/onboard next agentic dual-auth` (aliases `candidacy` / `list-org` / `org-installs` / `dual_auth` / `dual-auth-candidacy` as 4th token) → `MeshAgentOnboardingNextAgenticDualAuthCandidacy` — `list_org` fail-open · **tool ship ≠ dual-auth live** · **do not steal** dogfood soft aliases.
- **Lane status board (s1382+s1397+s1402+s1407+s1417+s1422+s1427):** `/onboard next status` (aliases `pulse` / `board`) → `MeshAgentOnboardingNextLaneStatus` — plugins dogfood lane reflects **session soft** marker after `/plugins dogfood` · mesh row is `streams_not_probed` · memory-pull/ops_pack row is `pull_not_probed` · agentic row is `list_plan_not_connected` · `portal_hitl_still` · **`<soft label>`** (s1422) · `dual_auth_candidacy_open` · `list_org_unavailable` (s1427).
- **Status export receipt (s1387+s1397+s1402+s1407+s1417+s1422+s1427):** `/onboard next export` (aliases `receipt` / `stamp` / `evidence`) → `MeshAgentOnboardingNextLaneStatusExport` · optional `/onboard next export json` → `MeshAgentOnboardingNextLaneStatusExportJSON` — same session soft + mesh + memory-pull + agentic soft + dual-auth candidacy rows · JSON field `agentic_list_plan_soft_state`.
- **Human-gates honesty board + still-human APPLY soft dogfood (s1413+s1546+s1550+s1574 Wave C continuum):** `/onboard next human-gates` (aliases `human` / `gates` / `apply-gates` / `still-human` / `apply-residual`) → `MeshAgentHumanGatesHonestyBoard` · soft `/onboard next human-gates dogfood` (aliases `soft` / `samples` / `offline` / `still-human-soft` / `apply-soft`) → `RunStillHumanApplySoftDogfood` — still-human APPLY open inventory residual after Wave A–C continuum · session labels `still_human_soft_not_run` · `soft_offline_still_human_session_pass` · `soft_offline_still_human_session_fail` · PASS ≠ invent human-gate green · PASS ≠ live APPLY · open boxes stay open · edge-first · knowledge multi-tenant punted · Slack HMAC punted · portal HITL when connect · dual_write OFF · Cloud Memory GA · residual PASS ≠ invent Connected · E10 Open · free eng **s1574** · free-floor peer **s1576+** mention only.
...
| mesh | `/onboard next mesh` | `stream` · `streams` · `heartbeat` · `heartbeats` · `pull` | `MeshAgentOnboardingNextMeshLane` |
| memory-pull | `/onboard next memory-pull` | `ops-pack` · `pull-path` · `memorypull` · `ops_pack` | `MeshAgentOnboardingNextMemoryPullLane` |
| agentic | `/onboard next agentic` | `agentic-integrations` · `integrations` · `list-plan` | `MeshAgentOnboardingNextAgenticLane` |
| agentic dual-auth | `/onboard next agentic dual-auth` | `candidacy` · `list-org` · `org-installs` · `dual_auth` · `dual-auth-candidacy` | `MeshAgentOnboardingNextAgenticDualAuthCandidacy` |
| portal-hitl | `/onboard next portal-hitl` | `hitl` · `portal_hitl` · `portal-dogfood` · `stage5` · `connectors-hitl` | `MeshAgentOnboardingNextPortalHITLLane` |
| portal-hitl dogfood | `/onboard next portal-hitl dogfood` | `soft` · `samples` · `offline` · `portal-hitl-soft` | `RunPortalHITLSoftDogfood` |
| e4 | `/onboard next e4` | `e4-dogfood` · `client-attach` · `edge-memory-e4` · `e4_attach` | `MeshAgentOnboardingNextE4Lane` |
| e4 dogfood | `/onboard next e4 dogfood` | `soft` · `samples` · `offline` · `e4-soft` | `RunE4SoftDogfood` |
| tool-call | `/onboard next tool-call` | `tool-calls` · `deeper-e4` · `e4-tools` · `ingest-retrieve` · `tool_call` | `MeshAgentOnboardingNextToolCallLane` |
| tool-call dogfood | `/onboard next tool-call dogfood` | `soft` · `samples` · `offline` · `tool-call-soft` | `RunDeeperToolCallSoftDogfood` |
| e10 | `/onboard next e10` | `e10-open` · `edge-memory-e10` · `ga-signoff` · `e10_open` | `MeshAgentOnboardingNextE10Lane` |
| e10 dogfood | `/onboard next e10 dogfood` | `soft` · `samples` · `offline` · `e10-soft` · `residual-check` | `RunE10OpenSoftDogfood` |
| planes | `/onboard next planes` | `three-planes` · `product-planes` · `product` · `pillars` · `three_planes` | `MeshAgentOnboardingNextThreePlanes` |
| sales | `/onboard next sales` | `claims` · `buyer` · `claim-matrix` · `sales-claims` · `buyer-claims` | `MeshAgentOnboardingNextSalesClaims` |
| demo | `/onboard next demo` | `demo-ready` · `readiness` · `demo-readiness` · `lighthouse` · `landgrab` | `MeshAgentOnboardingNextDemoReadiness` |
| marketing-demo | `/onboard next marketing-demo` | `marketing` · `sales-demo` · `demo-script` · `gtm-demo` | `MeshAgentOnboardingNextMarketingDemoLane` |
| operator | `/onboard next operator` | `operator-matrix` · `ops-matrix` · `operator-readiness` · `ops-readiness` · `matrix` | `MeshAgentOnboardingNextOperatorMatrix` |
| setup | `/onboard next setup` | `setup-lifecycle` · `lifecycle` · `setup_lifecycle` | `MeshAgentOnboardingNextSetupLane` |
| journey | `/onboard next journey` | `edge-journey` · `user-journey` · `first-run` · `edge_user_journey` | `MeshAgentOnboardingNextJourneyLane` |
| wizard | `/onboard next wizard` | `first-run-wizard` · `guided` · `wave-c` · `wave_c` · `wizard-residual` | `MeshAgentOnboardingNextWizardLane` |
| wizard dogfood | `/onboard next wizard dogfood` | `soft` · `samples` · `offline` · `wizard-soft` | `RunFirstRunWizardSoftDogfood` |
| human-gates | `/onboard next human-gates` | `human` · `gates` · `apply-gates` · `still-human` · `apply-residual` | `MeshAgentHumanGatesHonestyBoard` |
...
| status | `/onboard next status` | `pulse` · `board` | `MeshAgentOnboardingNextLaneStatus` |
| export | `/onboard next export` | `receipt` · `stamp` · `evidence` | `MeshAgentOnboardingNextLaneStatusExport` (+ optional `json`) |

Unknown lane token → overview + usage hint listing `plugins|gtm|memory|mesh|memory-pull|agentic|portal-hitl|e4|tool-call|e10|planes|sales|demo|marketing-demo|operator|setup|journey|wizard|status|export|human-gates`. **Note:** `pulse` is reserved for the status board — it is **not** a mesh alias. **Bare `pull` stays mesh** (s1402) — it is **not** a memory-pull alias (s1407 uses `memory-pull` / `ops-pack` / `pull-path` / `memorypull` / `ops_pack`). **Bare `mcp` stays memory** under `/onboard next` — it is **not** an agentic alias. **Bare `portal` / `agent-mcp` stay portal handoff** under `/onboard` — they are **not** agentic or portal-hitl next-lane aliases (agentic uses `agentic`/`integrations`/`list-plan`; portal HITL uses `portal-hitl`/`hitl` s1562). **`e4` / `client-attach` stay E4 client-attach** (s1566) — not memory lane (memory uses `memory`/`mcp`/`palace`) · not tool-call (s1578 uses `tool-call` / `tool-calls` / `deeper-e4` / `e4-tools` / `ingest-retrieve` / `tool_call`) · not e10 (s1586 uses `e10` / `e10-open` / `edge-memory-e10` / `ga-signoff` / `e10_open`). **`product` / `planes` stay three-planes** — they are **not** sales claims aliases (sales uses `sales` / `claims` / `buyer` / `claim-matrix` / `sales-claims` / `buyer-claims`). **`sales` / `claims` stay sales claims** — they are **not** demo readiness aliases (demo uses `demo` / `demo-ready` / `readiness` / `demo-readiness` / `lighthouse` / `landgrab`; landgrab stays honesty **NOT READY**) · **not** marketing-demo (s1590 uses `marketing-demo` / `marketing` / `sales-demo` / `demo-script` / `gtm-demo`). **`demo` / `readiness` / `lighthouse` / `landgrab` stay demo board** — they are **not** operator matrix aliases (operator uses `operator` / `operator-matrix` / `ops-matrix` / `operator-readiness` / `ops-readiness` / `matrix`) · **not** marketing-demo path. **`export` / `receipt` stay export** — they are **not** operator matrix aliases.
...
1. **`iomesh plugins dogfood`** / **`/plugins dogfood`** — offline sample validate (`examples/agent-plugins/{hello-iome,iomesh-memory-mcp}` product primary · s1517).
   ...
   - residual PASS ≠ live dogfood · rates ~$88/$119 optional · package load ≠ Connected.
...
3. **local-primary Memory edge** — TUI OSS + `iomesh-memory-mcp` + `github.com/iome-sh/memory` kernel + local palace · dual_write **OFF**.
   - Package load · Cloud Memory GA · ≠ freemium palace · not freemium hosted palace · local-primary only · **Palace sunset**.
   ...
   - **Edge OSS Option A (s1453):** product MCP host = **`iomesh-memory-mcp` only** · control plane = **private** cloud broker/CP · kernel module `github.com/iome-sh/memory` · s1517 no in-tree residual private Memory sample.
   - **Public product attach (s1478):** both edge repos **public** · `go install github.com/iome-sh/iomesh-memory-mcp/cmd/iomesh-memory-mcp@main` · `go get github.com/iome-sh/memory@main` · **no GOPRIVATE** · attach streamable HTTP `http://127.0.0.1:8080/mcp` or stdio · docker compose still valid · dual_write **OFF** · control-plane / broker private · **Cloud Memory GA · public edge ≠ invent Connected** · **Cloud Memory GA · public OSS ≠ invent Connected** · **PASS ≠ invent full platform sidecar parity**.
   ...
   - **Product-only sample:** `examples/agent-plugins/iomesh-memory-mcp` (s1517 · control-plane / broker private).
   ...
   - E4 client attach residual (s1508+s1566): lean host HTTP → `iomesh mcp --connect` · tools=6 stamp residual · Cloud Memory GA · residual PASS ≠ invent Connected · E10 Open · tip ≠ invent forever-green product dogfood · soft `/onboard next e4 dogfood`.
...
3b. **I/O Mesh = streaming org heartbeats on governed `dept.*`** — residual-honest path only.
   ...
   - Pull honesty (cross-link): `iomesh memory pull` = mesh → local palace **egress** · dual_write **OFF** · not freemium hosted palace — full Ops Pack drill is **D3c memory-pull**.
   ...
   - API: `MeshAgentOnboardingNextMeshLane`.
...
3c. **Memory Ops Pack pull path** — residual-honest mesh → local palace **egress** only (optional commercial overlay · **not first-run required**).
   ...
   - **dual_write OFF** · **Palace sunset** · **pull ≠ freemium hosted palace** · **not freemium hosted palace**.
   ...
   - **package load ≠ Ops Pack entitlement** · package load ≠ Connected.
   ...
   - API: `MeshAgentOnboardingNextMemoryPullLane`.
...
3d. **Agentic integrations (product plane 3)** — residual-honest MCP list/plan + portal HITL continuum.
   ...
   - dual_write **OFF** · book-demo **OFF** · residual PASS ≠ live dogfood · PASS ≠ live APPLY · open boxes stay open · rates ~$88/$119 optional.
   ...
   - API: `MeshAgentOnboardingNextAgenticLane`.
...
3d-dual-auth. **Dual-auth candidacy depth** for org installs snapshot residual — static offline only.
   ...
   - API: `MeshAgentOnboardingNextAgenticDualAuthCandidacy`.
...
3e. **Three product planes residual-honest consolidate** — offline static board for mesh · memory-pull · agentic narrative planes.
   ...
   - API: `MeshAgentOnboardingNextThreePlanes`.
...
3f. **Sales / buyer claims residual-honest matrix** — offline static board for founders/sales operators (may claim vs must not claim).
   ...
   - **Must not claim:** invent Connected / install APPLY green / INSTALL_STORE green · dual_write ON · freemium hosted palace · Ops Pack = GPU fleet · book-demo ON (book-demo **OFF**) · dual-auth live · empty-as-none invent · agent MCP write installs · invent Knowledge/Analytics GA · invent human-gate green (Slack HMAC · Stripe Customers:Write · H1/H2 still human).
   - Honesty locks: drafts only for GTM · book-demo **OFF** · dual_write **OFF** · never invent Connected · residual PASS ≠ live dogfood · PASS ≠ live APPLY · open boxes stay open · rates ~$88/$119 optional · `dual_auth_candidacy_open` · tool ship ≠ dual-auth live · mesh ≠ memory · Cloud Memory GA.
   ...
   - API: `MeshAgentOnboardingNextSalesClaims`.
...
3g. **Demo readiness residual-honest package** — offline static board for founders/operators (Lighthouse packaging · book-demo OFF · Landgrab NOT READY).
   ...
   - Honesty locks: dual_write **OFF** · book-demo **OFF** · Landgrab **NOT READY** · never invent Connected · residual PASS ≠ live dogfood · PASS ≠ live APPLY · open boxes stay open · rates ~$88/$119 optional · dual_auth_candidacy_open.
   - Slash: `/onboard next demo` (aliases `demo-ready` / `readiness` / `demo-readiness` / `lighthouse` / `landgrab`).
   - **Do not steal:** `sales` / `claims` stay sales claims · `product` / `planes` stay three-planes · `pulse` / `board` stay status board · `gtm` / `drafts` stay GTM lane · marketing-demo path uses `marketing-demo` / `marketing` / `sales-demo` / `demo-script` / `gtm-demo` (s1590).
   - Companion: `/onboard next planes` · `/onboard next sales` · `/onboard next marketing-demo` (s1590) · `/onboard next human-gates` · `/onboard next mesh` · `/onboard next memory-pull` · `/onboard next agentic` · `/onboard next status` · `/onboard next export`.

#### D3g2. Marketing demo path (`/onboard next marketing-demo` · s1590)

3g2. **Marketing demo path — plain-language operator script** for videos and sales demos of **local agent + local memory**.
   - Steps: (1) install/build iomesh · (2) set LLM key or Ollama · (3) `/setup init` local-memory + preflight · (4) start/attach `iomesh-memory-mcp` · (5) show `/memory` ingest + recall · (6) optional mesh only if configured.
   - dual_write **OFF** · local memory · mesh optional · **never invent Connected** · book-demo **OFF**.
   - Slash: `/onboard next marketing-demo` (aliases `marketing` / `sales-demo` / `demo-script` / `gtm-demo`).
   - **Do not steal:** bare `demo` stays demo readiness (s1442) · bare `sales` stays sales claims · bare `gtm` stays GTM drafts.
   - API: `MeshAgentOnboardingNextMarketingDemoLane`.
   - Docs: `internal note removed from this public repository` · companion `memory-edge-usage-demo.md` · `edge-user-journey.md`.
   - free eng **s1590** · free-floor peer **s1592+** mention only.
...
3i. **Setup lifecycle residual-honest map** — offline static consolidation of setup P1–P7 story (init · preflight · reload · portal · pull · analyze · drift · guided repair) · **stage 4** of edge-user-journey.
   ...
   - **repair apply ≠ invent Connected** · dual_write **OFF** · Cloud Memory GA · portal HITL · catalog ≠ Connected · **E10 Open** · still-human APPLY open · free eng **s1558**.
   ...
   - API: `MeshAgentOnboardingNextSetupLane`.
...
3k. **Portal HITL connectors (journey stage 5)** — residual-honest offline board for MCP list/plan → browser portal HITL → human OAuth/install.
   ...
   - **agent MCP cannot write installs** · **catalog ≠ Connected** · `template=` ≠ install APPLY · portal HITL still · dual_write OFF · book-demo OFF · Cloud Memory GA · residual PASS ≠ invent Connected.
   ...
   - API: `MeshAgentOnboardingNextPortalHITLLane` · `RunPortalHITLSoftDogfood` · `PortalHITLSoftSessionLabel`.
...
3l. **E4 client attach (journey stage 6 local store / MCP attach)** — residual-honest offline board for lean `iomesh-memory-mcp` → `iomesh mcp --connect` · tools=6 stamp residual.
   - Path: local-primary `iomesh-memory-mcp` · E4 client attach · tools=6 · `iomesh mcp --connect` residual · evidence `internal note removed from this public repository`.
   - **dual_write OFF** · Cloud Memory GA · residual PASS ≠ invent Connected · **E10 Open** · tip ≠ invent forever-green product dogfood · residual PASS ≠ live dogfood · PASS ≠ live APPLY · book-demo OFF.
   - Soft offline dogfood: `/onboard next e4 dogfood` (aliases 4th token `soft` / `samples` / `offline` / `e4-soft`) → `RunE4SoftDogfood`.
     ...
     - **never dial MCP · never start host** · soft offline ≠ invent Connected · session soft ≠ live dogfood · residual PASS ≠ live dogfood · residual PASS ≠ invent Connected · independent of portal HITL soft (s1562) and agentic list/plan soft (s1422).
   - Slash: `/onboard next e4` (aliases `e4-dogfood` / `client-attach` / `edge-memory-e4` / `e4_attach`) — **bare stays board** (not auto dogfood).
   - API: `MeshAgentOnboardingNextE4Lane` · `RunE4SoftDogfood` · `E4SoftSessionLabel`.
   - Companion: `/onboard next memory` · `/onboard next journey` · `/onboard next memory-pull` · `/onboard next tool-call` (s1578) · `/memory status`.
   - free eng **s1566** · free-floor peer **s1568+** mention only (do not rewrite free-floor).

#### D3m. Deeper tool-call residual + soft dogfood (`/onboard next tool-call` · s1578)

3m. **Deeper tool-call residual (journey stage 6/7 depth after E4 attach)** — residual-honest offline operator map for ingest → retrieve → list → as-of/status after tools=6 attach stamp.
   - Path: companion `/onboard next e4` · tools=6 · `iomesh mcp --connect` · s1508/s1566 attach stamp residual · tool names `memory_ingest_turn` · `memory_retrieve` · `memory_search_semantic` · `memory_list` · `memory_compact_status` · `memory_facts_as_of`.
   - **Partial→client-attach-evidence** · deeper tool-call residual candidacy only (not forever-green full product dogfood).
   - **dual_write OFF** · Cloud Memory GA · residual PASS ≠ invent Connected · **E10 Open** · tip ≠ invent forever-green product dogfood · residual PASS ≠ live dogfood · PASS ≠ live APPLY · book-demo OFF.
   - Soft offline dogfood: `/onboard next tool-call dogfood` (aliases 4th token `soft` / `samples` / `offline` / `tool-call-soft`) → `RunDeeperToolCallSoftDogfood`.
     - Default session: `tool_call_soft_not_run`
     - Soft pass: `soft_offline_tool_call_session_pass`
     - Soft fail: `soft_offline_tool_call_session_fail`
     - **never dial MCP · never start host** · soft offline ≠ invent Connected · session soft ≠ live dogfood · residual PASS ≠ live dogfood · residual PASS ≠ invent Connected · independent of E4 soft (s1566) · still-human soft (s1574) · wizard/portal HITL/agentic soft.
   - Slash: `/onboard next tool-call` (aliases `tool-calls` / `deeper-e4` / `e4-tools` / `ingest-retrieve` / `tool_call`) — **bare stays board** (not auto dogfood).
   - API: `MeshAgentOnboardingNextToolCallLane` · `RunDeeperToolCallSoftDogfood` · `ToolCallSoftSessionLabel`.
   - Companion: `/onboard next e4` · `/onboard next e4 dogfood` · `/onboard next e10` (s1586) · `/onboard next memory` · `/onboard next journey` · `/memory status` · internal note removed from this public repository.
   - free eng **s1578** · free-floor peer **s1580+** mention only (do not rewrite free-floor).

#### D3n. E10 Open reaffirm residual-check (`/onboard next e10` · s1586)

3n. **E10 Open reaffirm residual-check (Platform residual honesty after OSS packaging)** — residual-honest offline board pinning E10 Open · never invent E10 closed / Connected / live APPLY green.
   - Path: pin **E10 Open** · residual PASS ≠ invent E10 closed · residual PASS ≠ invent Connected · Cloud Memory GA · E10 stays Open · Cloud Memory GA ≠ close E10 · PASS ≠ live APPLY · dual_write OFF · book-demo OFF.
   - Soft offline residual-check: `/onboard next e10 dogfood` (aliases 4th token `soft` / `samples` / `offline` / `e10-soft` / `residual-check`) → `RunE10OpenSoftDogfood`.
     - Default session: `e10_soft_not_run`
     - Soft pass: `soft_offline_e10_session_pass`
     - Soft fail: `soft_offline_e10_session_fail`
     - **never dial MCP · never start host** · soft offline ≠ invent Connected · session soft ≠ live dogfood · residual PASS ≠ live dogfood · residual-check · independent of tool-call/E4/still-human/wizard soft.
   - Slash: `/onboard next e10` (aliases `e10-open` / `edge-memory-e10` / `ga-signoff` / `e10_open`) — **bare stays board** (not auto residual-check).
   - API: `MeshAgentOnboardingNextE10Lane` · `RunE10OpenSoftDogfood` · `E10SoftSessionLabel`.
   - Companion: `/onboard next e4` · `/onboard next human-gates` · `/onboard next tool-call` · OSS packaging · MIT harness · not control plane · docs `docs/architecture/oss-packaging-boundary.md` · `docs/architecture/edge-user-journey.md` · `internal note removed from this public repository`.
...
3h. **Operator readiness residual-honest matrix** — offline static board consolidating demo · sales · planes · human-gates for founders/operators.
   ...
   - **Row 2 Sales claims:** may claim / must not claim residual-honest · companion `/onboard next sales` · never invent Connected · Cloud Memory GA / dual-auth live.
   ...
   - **Row 4 Human gates:** edge-first · knowledge multi-tenant punted · Slack HMAC punted · portal HITL when connect · companion `/onboard next human-gates` · dual_write OFF · Cloud Memory GA.
   ...
   - **Row 6 Policy locks:** dual_write **OFF** · leave ON_SIGNAL unset · rates ~$88/$119 optional · Cloud Memory GA.
   ...
   - Honesty locks: book-demo **OFF** · Landgrab **NOT READY** · dual_write **OFF** · never invent Connected · residual PASS ≠ live dogfood · PASS ≠ live APPLY · open boxes stay open · dual_auth_candidacy_open.
   ...
   - API: `MeshAgentOnboardingNextOperatorMatrix`.
...
5. **Residual-honest lane status board** — offline pulse of plugins · gtm · memory · mesh · memory-pull · agentic · portal.
   ...
   - **memory:** dual_write OFF · package load ≠ Connected · local-primary ≠ freemium palace · mesh ≠ memory · **s1453+s1458+s1463+s1469+s1478** edge OSS tip (`iomesh-memory-mcp` · public product attach · go install · no GOPRIVATE · docker compose still valid · dual_write OFF · control-plane / broker private · Cloud Memory GA · public edge ≠ invent Connected · Cloud Memory GA · public OSS ≠ invent Connected · PASS ≠ invent full platform sidecar parity · mesh optional for pull · Palace sunset).
...
6. **Residual-honest status export receipt** — offline markdown (or optional JSON) evidence of the s1382 lane status board (plugins lane includes s1397 session soft marker when set · mesh row s1402 · memory-pull/ops_pack row s1407 · agentic row s1417+s1422 soft · dual-auth candidacy s1427).
   ...
   - Markdown: `/onboard next export` (aliases `receipt` / `stamp` / `evidence`) → `MeshAgentOnboardingNextLaneStatusExport`.
   - JSON: `/onboard next export json` → `MeshAgentOnboardingNextLaneStatusExportJSON`.
...
7. **Residual-honest human-gates status** — edge-first launch residual vs punted/demoted vs offline residual only / shipped-or-policy · still-human APPLY open inventory reaffirm after Wave A–C continuum.
   - Slash: `/onboard next human-gates` (aliases `human` / `gates` / `apply-gates` / `still-human` / `apply-residual`) → `MeshAgentHumanGatesHonestyBoard`.
   - Soft offline dogfood: `/onboard next human-gates dogfood` (aliases 4th token `soft` / `samples` / `offline` / `still-human-soft` / `apply-soft`) → `RunStillHumanApplySoftDogfood`.
     ...
     - **never dial MCP · never start host** · soft offline ≠ invent Connected · session soft ≠ live dogfood · residual PASS ≠ live dogfood · PASS ≠ invent human-gate green · PASS ≠ live APPLY · open boxes stay open · residual PASS ≠ invent Connected · independent of wizard/E4/portal HITL/agentic soft.
   ...
   - **still_human_or_policy (edge-first launch residual · still-human APPLY open):** portal HITL OAuth/connect when customer must Connect · catalog ≠ Connected · book-demo OFF · leave ON_SIGNAL unset · dual_write OFF · Cloud Memory GA · E10 Open · residual PASS ≠ invent Connected · Cloud Memory GA · E10 stays Open · optional E4 edge dogfood if tightening claim beyond candidacy.
   ...
   - **offline_residual_only / shipped_or_policy:** setup lifecycle residual complete ≠ invent Connected · Wave A–C continuum residual ≠ invent human-gate green / live APPLY · agent MCP list/plan residual-honest · dual_write OFF · Cloud Memory GA · local memory / dual_write OFF do NOT invent Connected.
   - Operator: `/onboard next human-gates` · `/onboard next human-gates dogfood` · `/onboard next e10` (s1586 E10 Open reaffirm) · `/onboard next e10 dogfood` · `/onboard next wizard` · `/onboard next setup` · `/integrations list|plan|status` · **never invent Connected / INSTALL_STORE green / book-demo ON / human-gate green / E10 closed**.
...
| package load ≠ Connected | Loading iomesh-memory-mcp / Ops Pack local ≠ invent Connected / freemium palace |
...
| Cloud Memory GA · public edge ≠ invent Connected | Public edge packs · Cloud Memory GA / freemium palace |
| Cloud Memory GA · public OSS ≠ invent Connected | Public MIT edge modules ≠ invent multi-tenant platform Memory GA |
| PASS ≠ invent full platform sidecar parity | Lean tool surface may lag platform residual · attach residual ≠ invent full parity |
| control-plane / broker private | control plane stays private cloud broker/CP · not OSS edge pack |
| s1517 product-only | In-tree residual private Memory sample removed · control-plane / broker private |
...
| board/export evidence ≠ invent Connected | Lane status board + export receipt are offline residual evidence only — never invent Connected / APPLY · Cloud Memory GA |
...
| residual PASS ≠ invent Connected | E4 soft residual (s1566) · attach stamp · tip never invent Connected · Cloud Memory GA |
| E10 Open | E10 remains open · residual PASS ≠ invent E10 closed |
...
| knowledge/analytical stay Beta | Do not invent GA for knowledge or analytical mesh layers |
...
- Do **not** treat portal test invoke / mint key as Connected.
...
- Do **not** invent Connected / freemium palace from local `iomesh-memory-mcp` package load.
- Do **not** invent Connected / platform GA from public product attach residual (`iomesh-memory-mcp` go install · no GOPRIVATE · dual_write OFF · Cloud Memory GA · public edge ≠ invent Connected · Cloud Memory GA · public OSS ≠ invent Connected · control-plane / broker private · s1478).
- Cloud Memory GA. Do **not** invent full platform sidecar parity from lean edge host attach residual (`iomesh-memory-mcp` · dual_write OFF · PASS ≠ invent full platform sidecar parity).
- Cloud Memory GA. Do **not** invent live dogfood green from M3 edge dogfood tip residual (`iomesh-memory-mcp` compose/HTTP · offline dogfood tip ≠ invent live dogfood as green · s1463).
- Do **not** invent Connected / APPLY from board or export receipt evidence stamps.
...
- Do **not** treat E4 soft offline dogfood pass/fail as live dogfood or invent Connected / forever-green product dogfood / E10 closed / dual_write ON (s1566 · `e4_soft_not_run` default · never dial MCP · never start host · residual PASS ≠ invent Connected · tip ≠ invent forever-green product dogfood · residual PASS ≠ live dogfood).
- Do **not** treat E10 soft offline residual-check pass/fail as invent E10 closed / Connected / live APPLY green / live dogfood (s1586 · `e10_soft_not_run` default · never dial MCP · never start host · residual PASS ≠ invent E10 closed · residual PASS ≠ invent Connected · residual PASS ≠ live dogfood · residual-check).
...
- Do **not** steal bare `e4` / `tool-call` / `human-gates` for E10 — E10 uses `e10` / `e10-open` / `edge-memory-e10` / `ga-signoff` / `e10_open` (s1586).
...
- Cloud Memory GA. Do **not** invent Connected / dual-auth live / human-gate green / freemium palace / Ops Pack GPU from the sales/buyer claims board (s1437 · may claim residual-honest only).
...
- Cloud Memory GA. Do **not** invent book-demo ON / Landgrab READY / Connected / logos met from the demo readiness board (s1442 · Lighthouse packaging residual-honest · Landgrab **NOT READY** · residual PASS ≠ logos met).
...
- Cloud Memory GA. Do **not** invent Connected / dual-auth live / human-gate green / book-demo ON / Landgrab READY from the operator readiness matrix (s1447 · residual_only · path_ready · still_human · policy_off · not_ready · portal_hitl_still).
...
- System note inject on `AttachMCP`: `<mesh-onboarding>` via `MeshAgentOnboardingGuidanceNote` (s1363+s1368+s1372+s1377+s1382+s1387+s1402+s1407+s1413+s1417+s1432+s1437+s1442+s1447).
- Portal handoff block: `MeshAgentOnboardingPortalHandoff` · slash `/onboard portal` (aliases `agent-mcp` / `mcp`).
- Offline status: `MeshAgentOnboardingStatus` · slash `/onboard status` (no MCP dial) · cross-links `/onboard next status` (s1382+s1397+s1402+s1407+s1417+s1422+s1427) · `/onboard next export` (s1387+s1397+s1402+s1407+s1417+s1422+s1427) · `/onboard next mesh` (s1402) · `/onboard next memory-pull` (s1407) · `/onboard next agentic` (s1417) · `/onboard next agentic dogfood` (s1422) · `/onboard next agentic dual-auth` (s1427) · `/onboard next planes` (s1432) · `/onboard next sales` (s1437) · `/onboard next demo` (s1442) · `/onboard next operator` (s1447) · `/onboard next human-gates` (s1413).
- Post-onboard next lanes overview: `MeshAgentOnboardingNextLanes` · slash `/onboard next` (aliases `after` / `continue` / `lanes`) — plugins · gtm · memory local · mesh streaming · Ops Pack pull path · agentic integrations product plane 3 · three product planes board (s1432) · sales/buyer claims (s1437) · demo readiness (s1442) · operator readiness matrix (s1447) · portal HITL still (s1372+s1402+s1407+s1417) · human-gates still-required vs offline (s1413) · status board (s1382+s1397+s1402+s1407+s1417+s1422+s1427) · export receipt (s1387+s1397+s1402+s1407+s1417+s1422+s1427).
- Lane drills (s1377+s1402+s1407+s1417): `MeshAgentOnboardingNextPluginsLane` · `MeshAgentOnboardingNextGtmLane` · `MeshAgentOnboardingNextMemoryLane` · `MeshAgentOnboardingNextMeshLane` · `MeshAgentOnboardingNextMemoryPullLane` · `MeshAgentOnboardingNextAgenticLane` · slash `/onboard next [plugins|gtm|memory|mesh|memory-pull|agentic]` (aliases plugin|dogfood · drafts · mcp|palace · stream|streams|heartbeat|heartbeats|pull · ops-pack|pull-path|memorypull|ops_pack · agentic-integrations|integrations|list-plan).
- Three product planes board (s1432): `MeshAgentOnboardingNextThreePlanes` · slash `/onboard next planes` (aliases three-planes|product-planes|product|pillars|three_planes) · mesh · memory-pull · agentic residual-honest · streams_not_probed · pull_not_probed · list_plan_not_connected · dual_auth_candidacy_open · never invent Connected · do not steal pulse/board/pull/mcp.
- Sales/buyer claims board (s1437): `MeshAgentOnboardingNextSalesClaims` · slash `/onboard next sales` (aliases claims|buyer|claim-matrix|sales-claims|buyer-claims) · may claim / must not claim residual-honest · three-planes grounded · never invent Connected · Cloud Memory GA / dual-auth live · do not steal product/planes/gtm/pulse/board.
- Demo readiness board (s1442): `MeshAgentOnboardingNextDemoReadiness` · slash `/onboard next demo` (aliases demo-ready|readiness|demo-readiness|lighthouse|landgrab) · Lighthouse beachhead packaging · book-demo OFF · Landgrab NOT READY · three planes · sales claims · human gates still open · residual PASS ≠ logos met · never invent Connected / book-demo ON · do not steal sales/claims/product/planes/pulse/board/gtm/drafts.
- Operator readiness matrix (s1447): `MeshAgentOnboardingNextOperatorMatrix` · slash `/onboard next operator` (aliases operator-matrix|ops-matrix|operator-readiness|ops-readiness|matrix) · demo · sales · planes · human-gates · dual-auth candidacy · policy locks residual-honest · residual_only · path_ready · still_human · policy_off · not_ready · portal_hitl_still · never invent Connected / GA · do not steal demo/readiness/lighthouse/landgrab/sales/claims/product/planes/pulse/board/export/receipt.
- Setup lifecycle map (s1542+s1558 · stage 4): `MeshAgentOnboardingNextSetupLane` · slash `/onboard next setup` (aliases setup-lifecycle|lifecycle|setup_lifecycle) · P1–P7 closeout residual · setup_not_probed · package wire ≠ Connected · repair apply ≠ invent Connected · dual_write never auto ON · E10 Open · offline static ≠ live dogfood · Cloud Memory GA · setup closeout ≠ invent Connected · companion `/setup` slash + skill `setup-lifecycle-agent` · full first-run `/onboard next journey`.
- Edge-user-journey first-run map (s1558 Wave B): `MeshAgentOnboardingNextJourneyLane` · slash `/onboard next journey` (aliases edge-journey|user-journey|first-run|edge_user_journey) · 7 stages residual-honest · dual_write OFF · Cloud Memory GA · residual PASS ≠ invent Connected · portal HITL · no invent TUI portal SSO · host not auto · free eng s1558 · free-floor peer s1560+ mention only · docs edge-user-journey · setup-lifecycle · memory-edge-usage-demo.
- Portal HITL stage-5 board (s1562): `MeshAgentOnboardingNextPortalHITLLane` · slash `/onboard next portal-hitl` (aliases hitl|portal_hitl|portal-dogfood|stage5|connectors-hitl) · journey stage 5 · portal HITL when connect · soft dogfood residual · free eng s1562 · free-floor peer s1564+ mention only.
...
- E4 client-attach stage-6 board (s1566): `MeshAgentOnboardingNextE4Lane` · slash `/onboard next e4` (aliases e4-dogfood|client-attach|edge-memory-e4|e4_attach) · journey stage 6 · E4 client attach · tools=6 · iomesh mcp --connect residual · free eng s1566 · free-floor peer s1568+ mention only.
- First-run wizard residual (s1570 Wave C): `MeshAgentOnboardingNextWizardLane` · slash `/onboard next wizard` (aliases first-run-wizard|guided|wave-c|wave_c|wizard-residual) · soft `/onboard next wizard dogfood` (aliases soft|samples|offline|wizard-soft) → `RunFirstRunWizardSoftDogfood` · default `wizard_soft_not_run` · after run `soft_offline_wizard_session_pass` | `soft_offline_wizard_session_fail` · residual PASS ≠ invent full interactive auto wizard · residual PASS ≠ invent Connected · free eng s1570 · free-floor peer s1572+ mention only · bare wizard stays board · independent of E4/portal HITL/agentic soft · companion still-human soft s1574.
- E4 soft offline dogfood (s1566): `RunE4SoftDogfood` · slash `/onboard next e4 dogfood` (aliases soft|samples|offline|e4-soft) · default `e4_soft_not_run` · never dial MCP · never start host · residual PASS ≠ invent Connected · E10 Open · tip ≠ invent forever-green product dogfood · residual PASS ≠ live dogfood · bare e4 stays board · independent of portal HITL soft and agentic list/plan soft.
- Deeper tool-call residual (s1578): `MeshAgentOnboardingNextToolCallLane` · slash `/onboard next tool-call` (aliases tool-calls|deeper-e4|e4-tools|ingest-retrieve|tool_call) · soft `/onboard next tool-call dogfood` (aliases soft|samples|offline|tool-call-soft) → `RunDeeperToolCallSoftDogfood` · default `tool_call_soft_not_run` · after run `soft_offline_tool_call_session_pass` | `soft_offline_tool_call_session_fail` · never dial MCP · never start host · residual PASS ≠ invent Connected · E10 Open · tip ≠ invent forever-green product dogfood · residual PASS ≠ live dogfood · Partial→client-attach-evidence · free eng s1578 · free-floor peer s1580+ mention only · bare tool-call stays board · independent of E4/still-human/wizard/portal HITL/agentic soft.
- E10 Open reaffirm residual-check (s1586): `MeshAgentOnboardingNextE10Lane` · slash `/onboard next e10` (aliases e10-open|edge-memory-e10|ga-signoff|e10_open) · soft residual-check `/onboard next e10 dogfood` (aliases soft|samples|offline|e10-soft|residual-check) → `RunE10OpenSoftDogfood` · default `e10_soft_not_run` · after run `soft_offline_e10_session_pass` | `soft_offline_e10_session_fail` · never dial MCP · never start host · residual PASS ≠ invent E10 closed · residual PASS ≠ invent Connected · E10 Open · Cloud Memory GA · E10 stays Open · Cloud Memory GA ≠ close E10 · PASS ≠ live APPLY · free eng s1586 · free-floor peer s1588+ mention only · bare e10 stays board · independent of tool-call/E4/still-human soft.
...
- Slash residual honesty: `/onboard [help|checklist|portal|status|next]` · `/onboard next [plugins|gtm|memory|mesh|memory-pull|agentic|portal-hitl|e4|tool-call|e10|planes|sales|demo|marketing-demo|operator|setup|journey|wizard|status|export|human-gates]` · `/onboard next portal-hitl dogfood` · `/onboard next e4 dogfood` · `/onboard next tool-call dogfood` · `/onboard next e10 dogfood` · `/onboard next wizard dogfood` · `/onboard next human-gates dogfood` · `/onboard next agentic dogfood` · `/onboard next agentic dual-auth` · `/plugins dogfood|status` · `/integrations list|plan|status|signing` · `/memory status` · `/mesh` · `/gtm [help|checklist]` · `/setup [init|preflight|portal|reload|pull|analyze|drift|repair]`.
- Skills are **not** Agent Plugins — plugins dogfood ≠ invent Agent Plugins GA · session soft ≠ live dogfood · mesh ≠ memory · Ops Pack ≠ GPU fleet · human-gates offline ≠ invent APPLY · still-human soft ≠ invent human-gate green / live APPLY · agentic list/plan ≠ invent Connected · soft offline list/plan ≠ invent Connected · E4 soft ≠ invent Connected / forever-green product dogfood · tool-call soft ≠ invent Connected / forever-green product dogfood / live dogfood · E10 soft residual-check ≠ invent E10 closed / Connected / live APPLY · tool ship ≠ dual-auth live · three planes ≠ invent stream/pull/Connected green · sales claims ≠ invent Connected/Memory GA/dual-auth live · operator matrix ≠ invent Connected/GA/dual-auth live/book-demo ON · marketing demo path ≠ invent Connected/Memory GA/dual_write ON · edge-user-journey residual map ≠ invent SSO/auto host/Memory GA/Connected.
- Marketing demo path (s1590): `MeshAgentOnboardingNextMarketingDemoLane` · slash `/onboard next marketing-demo` (aliases marketing|sales-demo|demo-script|gtm-demo) · plain-language local agent + local memory script · dual_write OFF · mesh optional · never invent Connected · free eng s1590 · internal note removed from this public repository.
