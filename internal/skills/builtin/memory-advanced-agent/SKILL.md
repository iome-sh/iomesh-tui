...
| no invent GA | No invent GA window engine, lean HTTP for supersede/facts-as-of/patterns, or empty-as-success |
...
| empty ≠ invent | Empty facts / zero superseded_count / empty digest = honest empty |

## Non-goals (never do)

- Do **not** auto-run multi-hop related on default auto-recall.
- Do **not** silent supersede (always HITL / `--i-confirm`).
- Do **not** invent lean HTTP for supersede, facts-as-of, or patterns/anomalies.
...
- Do **not** invent memories, superseded_count, or digests when offline / empty.
