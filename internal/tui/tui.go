			...
			case "next", "after", "continue", "lanes":
				...
				if len(parts) >= 3 {
					...
					case "memory", "mcp", "palace":
						// s1377+s1453+s1458+s1463+s1469: local-primary memory + edge OSS Option A + M2 lean attach + M3 edge dogfood + M4 public flip readiness.
						fmt.Fprintln(out, agent.AionAgentOnboardingNextMemoryLane())
						fmt.Fprintln(out, "— residual: memory local lane · dual_write OFF · not Memory GA · package load ≠ Memory GA · ≠ freemium palace · mesh ≠ memory · iomesh-memory-mcp · M2 lean attach · scaffold/M2 · M3 edge dogfood · docker compose · 8080/mcp · healthz · M4 public flip readiness · kernel first · still private · residual PASS ≠ public flip · readiness tip ≠ invent public flip complete · offline dogfood tip ≠ invent live dogfood as green · PASS ≠ invent full platform sidecar parity · aion broker private · OSS path ≠ invent public flip complete · portal HITL")
						return false, nil
