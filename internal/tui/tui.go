...
func handleSlash(out io.Writer, rt runtimeAdapter, line string) (quit bool, err error) {
	...
	case "/onboard", "/aion-onboard", "/agent-onboard":
		...
		if len(parts) >= 2 {
			...
			case "next", "after", "continue", "lanes":
				...
				if len(parts) >= 3 {
					...
					case "tool-call", "tool-calls", "deeper-e4", "e4-tools", "ingest-retrieve", "tool_call":
						...
						// NOT bare e4|client-attach (E4 attach lane) · NOT bare mcp|palace (memory lane).
						if len(parts) >= 4 {
							sub := strings.ToLower(parts[3])
							switch sub {
					...
					case "human-gates", "human", "gates", "apply-gates", "still-human", "apply-residual":
						...
						fmt.Fprintln(out, "— residual: human-gates honesty board · s1550 edge-first · s1574 Wave C continuum still-human APPLY residual · dual_write OFF · not Memory GA · Edge Memory GA candidacy only · residual PASS ≠ invent Edge Memory GA · residual PASS ≠ invent Edge Memory GA declared · PASS ≠ invent Connected · PASS ≠ invent human-gate green · PASS ≠ live APPLY · open boxes stay open · knowledge multi-tenant punted · Slack HMAC punted · portal HITL when connect · book-demo OFF · leave ON_SIGNAL unset · H1/H2 not launch gate · E10 Open · agent MCP cannot write installs · never invent Connected / INSTALL_STORE green / book-demo as ON · soft dogfood: /onboard next human-gates dogfood · free eng s1574 · free-floor peer s1576+ mention only")
						return false, nil
					...
					default:
						...
						fmt.Fprintln(out, "— residual: post-onboard next lanes · dual_write OFF · not Memory GA · plugins dogfood ≠ Agent Plugins GA · drafts only · no auto-send · package load ≠ Memory GA · mesh ≠ memory · portal HITL · board/export evidence ≠ invent Connected · pull_not_probed · list_plan_not_connected · setup_not_probed · PASS ≠ invent human-gate green · Edge Memory GA candidacy only · free eng s1558 · free eng s1562 · free eng s1566 · free eng s1570 · free eng s1574 · free eng s1578 · free eng s1582 · OSS packaging residual")
						fmt.Fprintln(out, "— packaging: "+agent.OSSPackagingHonestyOneLiner)
						fmt.Fprintln(out, "usage: /onboard next [plugins|gtm|memory|mesh|memory-pull|agentic|portal-hitl|e4|tool-call|planes|sales|demo|operator|setup|journey|wizard|status|export|human-gates]  (Edge OSS path: setup|journey|wizard|memory|e4|portal-hitl · Platform residual honesty optional residual-check: human-gates|tool-call · soft residual-check = offline residual honesty · slash dogfood kept for compatibility; lane aliases: plugins→plugin|dogfood · gtm→drafts · memory→mcp|palace · mesh→stream|streams|heartbeat|heartbeats|pull · memory-pull→ops-pack|pull-path|memorypull|ops_pack · agentic→agentic-integrations|integrations|list-plan · portal-hitl→hitl|portal_hitl|portal-dogfood|stage5|connectors-hitl · e4→e4-dogfood|client-attach|edge-memory-e4|e4_attach · tool-call→tool-calls|deeper-e4|e4-tools|ingest-retrieve|tool_call · planes→three-planes|product-planes|product|pillars|three_planes · sales→claims|buyer|claim-matrix|sales-claims|buyer-claims · demo→demo-ready|readiness|demo-readiness|lighthouse|landgrab · operator→operator-matrix|ops-matrix|operator-readiness|ops-readiness|matrix · setup→setup-lifecycle|lifecycle|setup_lifecycle · journey→edge-journey|user-journey|first-run|edge_user_journey · wizard→first-run-wizard|guided|wave-c|wave_c|wizard-residual · status→pulse|board · export→receipt|stamp|evidence · human-gates→human|gates|apply-gates|still-human|apply-residual; soft residual-check dogfood: tool-call dogfood|soft|samples|offline|tool-call-soft · human-gates dogfood|soft|samples|offline|still-human-soft|apply-soft; parent aliases after|continue|lanes; export json for JSON receipt; pulse stays status board; bare pull stays mesh; bare mcp stays memory; bare portal stays portal handoff; product/planes stay three-planes; landgrab stays Landgrab NOT READY honesty; readiness/lighthouse stay demo board)")
						return false, nil
					}
				}
				fmt.Fprintln(out, agent.AionAgentOnboardingNextLanes())
			...
			fmt.Fprintln(out, "— packaging: "+agent.OSSPackagingHonestyOneLiner)
			fmt.Fprintln(out, "usage: /onboard [help|checklist|portal|status|next]  (aliases /aion-onboard /agent-onboard; portal aliases agent-mcp|mcp; next aliases after|continue|lanes; next lanes Edge OSS path: setup|journey|wizard|memory|e4|portal-hitl · Platform residual honesty (optional residual-check): human-gates|tool-call · also plugins|gtm|mesh|memory-pull|agentic|planes|sales|demo|operator|status|export)")
			return false, nil
		}
		fmt.Fprintln(out, agent.AionAgentOnboardingGuidanceNote())
		fmt.Fprintln(out, "— residual: TUI ↔ aion onboarding · dual_write OFF · not Memory GA · never invent Connected · portal HITL · skill aion-agent-onboarding via read_skill")
