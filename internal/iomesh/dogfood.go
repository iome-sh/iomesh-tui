...
type DogfoodReport struct {
	...
	// WaitReadyAttempts is the WaitReady probe cycle count from WaitReadyAttempts
	// (0 when WaitReady budget off / step not run). Always emitted so CI sees how many
	// Health/Ready loops ran without scraping step detail.
	WaitReadyAttempts int `json:"wait_ready_attempts"`
	...
	// KVListMS is KVListKeys-only latency in milliseconds
	// (0 when kv probe unset or list not run). Always emitted in JSON.
	// Distinct from KVMS (full step latency: ensure + list) and KVEnsureMS.
	KVListMS int `json:"kv_list_ms"`
...
func (c *Client) Dogfood(ctx context.Context, opts DogfoodOptions) DogfoodReport {
	...
	if opts.WaitReady > 0 {
		...
		// Capture probe cycles from WaitReadyAttempts for always-emit wait_ready_attempts.
		...
		waitReadyStep := c.stepTimed("wait_ready", func() (StepStatus, string) {
			...
			defer cancel()
			n, err := c.WaitReadyAttempts(wctx, WaitReadyOptions{
				Interval:      interval,
				RequireHealth: opts.WaitRequireHealth,
			})
			waitAttempts = n
			if err == nil {
				detail := fmt.Sprintf("WaitReady OK budget=%s interval=%s attempts=%d", opts.WaitReady, interval, n)
		...
		rep.WaitReadyAttempts = waitAttempts
	...
	// (0 when ensure off / probe unset / not attempted); KVListMS is list-keys only
	// (0 when probe unset / list not run).
	...
	if kvBucket == "" {
		...
		kvStep := c.stepTimed("kv", func() (StepStatus, string) {
			...
			rep.KVListMS = int(time.Since(listStart).Milliseconds())
			if err != nil {
				rep.KVKeyCount = 0
				if opts.Strict {
...
func FormatReportJSON(r DogfoodReport) string {
	...
	type out struct {
		...
		KVListMS             int        `json:"kv_list_ms"`             // always emit (0 when kv probe unset / list not run)
