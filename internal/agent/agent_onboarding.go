...
// Agent Plugins GA, or dual_write ON.
...
// "residual-check" alongside slash token dogfood. Never invents control plane · Cloud Memory GA /
// dual_write ON / book-demo ON / live dogfood green.
const OSSPackagingHonestyOneLiner = "MIT OSS harness · not control plane · dual_write OFF · Cloud Memory GA · book-demo OFF · residual PASS ≠ invent control plane in MIT repo · soft residual-check (… dogfood slash) = offline residual honesty check · session soft ≠ live dogfood · ≠ invent platform green · free eng s1582 · free-floor peer s1584+ mention only · docs/architecture/oss-packaging-boundary.md"

// MeshAgentOnboardingLegacyHiddenOneLiner is the optional default-surface footnote.
// Hidden slash/lanes still work if typed; they are not the TTFH start-here.
const MeshAgentOnboardingLegacyHiddenOneLiner = "legacy (hidden): /gtm · /plugins · /onboard next plugins|gtm|marketing-demo|sales|demo|e4|e10|human-gates|planes|agentic|tool-call|portal-hitl|memory-pull|operator|wizard|export"
...
// Cloud Memory GA. Never invent Connected · Cloud Memory GA from onboard maps alone.
...
func OnboardNextStepLines() []string {
	return []string{
		...
		"note: dual_write OFF · package wire ≠ Connected · catalog ≠ Connected · agent MCP cannot write installs · Cloud Memory GA · never invent Connected",
	}
}
...
// dual_write OFF · CLIENT ≠ PULSE · catalog ≠ Connected · Cloud Memory GA.
...
// Connected · Cloud Memory GA / install APPLY.
func MeshAgentOnboardingStartHere() string {
  ...
  Walk (one list · TTFH rollout · mesh not required for R0–R2):
    setup. LLM key or Ollama · /setup init local-memory · /setup preflight · /setup reload · attach iomesh-memory-mcp (Cloud Memory GA)
...
Locks (never violate):
- dual_write OFF · never invent Connected · catalog ≠ Connected · Cloud Memory GA
- never invent install green / INSTALL_STORE APPLY
- knowledge Beta empty · eval template · empty until consume · not live APPLY
- PULSE only after ≥1 decoded broker message · CLIENT ≠ PULSE
- digest miss is success · miss ≠ known until /dashboard ack · no send/pay/ship
- agent MCP cannot write installs · portal HITL when connect
- package load ≠ Connected · local-primary`)
}
...
// install green, Agent Plugins GA, dual_write ON, or agent APPLY.
...
func MeshAgentOnboardingChecklist() string {
	return MeshAgentOnboardingStartHere() + "\n\n" + strings.TrimSpace(`mesh agent onboarding checklist (I/O Mesh TTFH · residual-honest):
  1. LLM key or Ollama
  2. /setup init local-memory · /setup preflight · /setup reload (CLI: iomesh setup preflight)
  3. Attach iomesh-memory-mcp (local palace · dual_write OFF · Cloud Memory GA)
...
`+MeshAgentOnboardingLegacyHiddenOneLiner+`
  Locks: dual_write OFF · catalog ≠ Connected · Cloud Memory GA · never invent Connected · never invent install green / INSTALL_STORE APPLY · knowledge Beta empty · eval template · empty until consume · not live APPLY · agent MCP cannot write installs · portal HITL when connect · package load ≠ Connected`) + "\n" + strings.Join(OnboardNextStepLines(), "\n")
}
...
// Never invents install green, or agent write installs.
...
Portal half (browser HITL · https://console.iome.sh/settings/agent):
  ...
  3. Test invoke = stub|live probe · 42ms/no preview = stub · ≠ live tools/call · ≠ consume · Cloud Memory GA
...
Locks: dual_write OFF · book-demo OFF · residual PASS ≠ live dogfood · never invent install green / Connected / INSTALL_STORE APPLY · list_org fail-open ≠ empty-as-none · catalog ≠ Connected · portal HITL · plugins dogfood ≠ invent Agent Plugins GA · Cloud Memory GA · setup closeout ≠ invent Connected · still-human APPLY open · setup closeout residual ≠ invent APPLY · Cloud Memory GA · residual PASS ≠ invent Connected · E10 Open · free eng s1562 · free eng s1566 · free eng s1570 · free-floor peer s1564+ mention only · free-floor peer s1568+ mention only · free-floor peer s1572+ mention only`) + "\n" + strings.Join(OnboardNextStepLines(), "\n")
}
...
// No MCP dial — operator pulse only. Never invents attach green, install Connected, · Cloud Memory GA.
...
func MeshAgentOnboardingStatus() string {
	return MeshAgentOnboardingStartHere() + "\n\n" + strings.TrimSpace(`mesh onboard status (I/O Mesh TTFH · residual-honest · offline static · no MCP dial):
  TTFH: setup → ingest RCA → optional consume/dashboard → cite-both digest
  dual_write OFF · local-primary · catalog ≠ Connected · Cloud Memory GA · never invent Connected
  /dashboard empty until consume · PULSE only after ≥1 decoded broker message · CLIENT ≠ PULSE
  knowledge Beta empty · eval template · not live APPLY
  digest miss is success · miss ≠ known until /dashboard ack · no send/pay/ship
  agent MCP cannot write installs · portal HITL when connect · package load ≠ Connected
  slash: /onboard next ttfh · /onboard next setup · /onboard next memory · /onboard next mesh · /onboard checklist
`+MeshAgentOnboardingLegacyHiddenOneLiner) + "\n" + strings.Join(OnboardNextStepLines(), "\n")
}

// MeshAgentOnboardingNextLanes is the default /onboard next surface: I/O Mesh TTFH
// lanes only (setup · memory · mesh · dashboard/digest). Hidden drills still exist
// as NextPluginsLane / NextGtmLane / … and work if typed.
// Never invents Connected, dual_write ON, or live APPLY.
...
Locks: dual_write OFF · catalog ≠ Connected · Cloud Memory GA · never invent Connected · knowledge Beta empty · eval template · empty until consume · not live APPLY · CLIENT ≠ PULSE · package load ≠ Connected · agent MCP cannot write installs`) + "\n" + strings.Join(OnboardNextStepLines(), "\n")
}
...
func MeshAgentOnboardingNextTTFHLane() string {
  ...
  Walk (one list · TTFH rollout · mesh not required for R0–R2):
    setup. LLM key or Ollama · /setup init local-memory · attach iomesh-memory-mcp (Cloud Memory GA)
    R0  iomesh ttfh --unit                 offline · no mesh · not E-G1
    R1  optional iomesh ttfh --live        fail-open probe · EMPTY unless decoded · not overlay PULSE
        Optional mesh: IOMESH_ENDPOINT for R1 only. Overlay /dashboard consume is R3, not this.
    R2  /memory ingest ×3 (source_hint=private)
        ...
        named miss: ` + ModeADigestNamedMissVocab + `
        /memory patterns (Beta) · empty ≠ invent · never APPLY · /memory facts-as-of --as-of <RFC3339> [--department support] [--department ops] [--department sales] [--department customer_success]
        Miss ACK: /dashboard ack (local ritual · no send/pay/ship)
        ...
        Sev-1 CS packet (PD-HMAC-5xx): scripts/sev1-cs-packet.sh · sev1-packet.md in examples/dept-rca/customer_success · ingest-dir --department customer_success · /memory digest --require-sources mesh,private (cite-both or named miss) · mesh miss is success · Zendesk optional · pulse does not exist · not E-G1
    R3  /dashboard consume                 entitled overlay PULSE (parked · required for E-G1)
        CLI: iomesh mesh smoke is not overlay PULSE. --live decoded-N is not this.
    R4  iomesh memory pull                 after PULSE · dual_write OFF · pull ≠ Connected
...
func MeshAgentOnboardingNextPluginsLane() string {
 ...
 · package load ≠ Connected · rates ~$88/$119 optional
...
Locks: dual_write OFF · book-demo OFF · residual PASS ≠ live dogfood · never invent install green / Connected / INSTALL_STORE APPLY · plugins dogfood ≠ invent Agent Plugins GA · plugins smoke ≠ invent Agent Plugins GA · catalog ≠ Connected · portal HITL · agent MCP cannot write installs · package load ≠ Connected · rates ~$88/$119 optional`)
}
...
Locks: dual_write OFF · book-demo OFF · residual PASS ≠ live dogfood · drafts only · no auto-send · human publish · GTM checklist ≠ invent GTM agent GA · never invent install green / Connected / INSTALL_STORE APPLY · catalog ≠ Connected · portal HITL · agent MCP cannot write installs · rates ~$88/$119 optional`)
}
...
// Cloud Memory GA · dual_write OFF · control-plane / broker still private · not freemium hosted palace.
// s1508: E4 full MCP client attach dogfood tip residual (connected=1 · tools=6 stamp) · Cloud Memory GA
// only · E10 Open · tip ≠ invent Connected · tip ≠ invent forever-green product dogfood.
...
// Never invents Connected, freemium palace, dual_write ON, install Connected, or live dogfood green.
func MeshAgentOnboardingNextMemoryLane() string {
  ...
  First-run honesty (s1695): OSS first-run complete without mesh · Ops Pack not first-run required · Memory Ops Pack optional · mesh optional · dual_write OFF · not freemium hosted palace
 ...
 · product attach path is iomesh-memory-mcp only (s1517) · package load ≠ Connected
  Steps:
    1. Public install product host: go install github.com/iome-sh/iomesh-memory-mcp/cmd/iomesh-memory-mcp@main — no GOPRIVATE · package load ≠ Connected · ≠ freemium palace
    2. Kernel (public): go get github.com/iome-sh/memory@main — no GOPRIVATE
    3. Attach local-primary: HTTP http://127.0.0.1:8080/mcp or stdio iomesh-memory-mcp · dual_write OFF
    ...
    5. dual_write OFF · local-primary only · Palace sunset · control-plane / broker private · control-plane still private · OSS first-run complete without mesh
    6. Optional: read_skill memory-advanced-agent (opt-in advanced · still dual_write OFF)
    ...
    9. Optional E4 client attach dogfood (s1508): iomesh mcp --connect after lean host HTTP — stamp residual · Cloud Memory GA · E10 Open · tip ≠ invent forever-green product dogfood
  Honesty:
 · package load ≠ Connected · ≠ freemium palace · not freemium hosted palace · dual_write OFF · Palace sunset
 · OSS first-run complete without mesh · Ops Pack not first-run required · Memory Ops Pack optional · mesh optional
 · residual PASS ≠ live dogfood · offline dogfood tip ≠ invent live dogfood as green · test invoke = probe only ≠ Connected · PASS ≠ live APPLY
 · PASS ≠ invent full platform sidecar parity · tool parity may be lean vs platform residual
 · public product attach (s1478) · no GOPRIVATE · go install · Cloud Memory GA · public edge ≠ invent Connected · Cloud Memory GA · public OSS ≠ invent Connected
 · E4 client attach (s1508) · Cloud Memory GA · residual PASS ≠ invent Connected · E10 Open · tip ≠ invent forever-green product dogfood · Cloud Memory GA
...
func MeshAgentOnboardingNextMeshLane() string {
 ...
 · pull ≠ freemium hosted palace · package load ≠ Connected · rates ~$88 mesh / ~$119 Memory Ops Pack optional
 · catalog ≠ Connected · portal HITL · agent MCP cannot write installs
...
// NOT freemium hosted palace · dual_write OFF · Cloud Memory GA · Palace sunset · Ops Pack ≠ GPU fleet.
...
func MeshAgentOnboardingNextMemoryPullLane() string {
  ...
  Honesty:
 · pull = mesh → local palace egress · dual_write OFF · not freemium hosted palace · Palace sunset
 · Ops Pack ≠ GPU fleet · package load ≠ Ops Pack entitlement · package load ≠ Connected
...
// dual_write OFF · Cloud Memory GA · catalog ≠ Connected · portal HITL · repair apply ≠ invent Connected ·
...
func MeshAgentOnboardingNextSetupLane() string {
  ...
  Product: setup lifecycle P1–P7 residual closeout (init · preflight · reload · continuous pull · analyze ticks · drift · guided repair) — offline static lane ≠ live dogfood · Cloud Memory GA · setup closeout ≠ invent Connected · full first-run map: /onboard next journey (s1558 Wave B) · guided residual: /onboard next wizard (s1570 Wave C)
  Steps:
  ...
  Honesty:
 · dual_write OFF · catalog ≠ Connected · portal HITL
 ...
 · residual PASS ≠ live dogfood · offline static lane ≠ live dogfood · Cloud Memory GA · setup closeout ≠ invent Connected
 ...
 · stage 4 of edge-user-journey · free eng s1558 · Cloud Memory GA
...
// Never invents: auto memory host · TUI portal SSO · Connected · dual_write ON ·
// agent install APPLY · book-demo ON. Cloud Memory GA.
...
func MeshAgentOnboardingNextJourneyLane() string {
  ...
  Stages (order residual-honest):
    1. Signup — portal console.iome.sh create/join org · optional pure local (skip for pure local memory) · signup ≠ Connected · ≠ Connected
    ...
    6. Local store iomesh-memory-mcp (journey stage 6) — install/run host + kernel · attach HTTP/stdio · dual_write OFF · host not auto on signup
       ...
       soft residual: /onboard next e4 dogfood (s1566 · soft offline ≠ invent Connected · residual PASS ≠ invent Connected · session soft ≠ live dogfood)
       deeper soft residual: /onboard next tool-call dogfood (s1578 · ingest→retrieve→list→as-of · soft offline ≠ invent Connected · residual PASS ≠ invent Connected)
...
// Never invents: auto memory host · TUI portal SSO · Connected · dual_write ON · agent install APPLY ·
// book-demo ON · Connected · forever-green interactive wizard UX.
...
func MeshAgentOnboardingNextWizardLane() string {
  ...
  Wave C residual scope: deeper guided residual map + soft dogfood · residual PASS ≠ invent full interactive auto wizard · residual PASS ≠ invent Connected
  Stages (guided residual · step + primary next action + honesty non-claim):
    1. Signup — portal console.iome.sh create/join org · optional pure local (skip for pure local memory)
       ...
       honesty: signup ≠ Connected · ≠ Connected · book-demo OFF
    ...
    6. Local store — stage 6 residual · /onboard next e4 · iomesh-memory-mcp attach
       ...
       honesty: dual_write OFF · Cloud Memory GA · host not auto · residual PASS ≠ invent Connected · E10 Open
    7. Analyze — /setup analyze · /memory digest · optional mesh Ops Pack pull
       ...
       honesty: analyze ≠ invent Connected · pull · Cloud Memory GA
 ...
 · soft offline ≠ invent Connected · residual PASS ≠ live dogfood · session soft ≠ live dogfood · residual PASS ≠ invent Connected
  Honesty:
 · Wave C · first-run wizard residual · free eng s1570 · free-floor peer s1572+ mention only
 · dual_write OFF · book-demo OFF · Cloud Memory GA · E10 Open
 · portal HITL when connect · agent MCP cannot write installs · catalog ≠ Connected
 · residual PASS ≠ invent Connected · residual PASS ≠ invent Connected
...
func MeshAgentOnboardingNextPortalHITLLane() string {
  ...
  Steps:
    ...
    4. Agent/MCP mint/copy/probe companion @ https://console.iome.sh/settings/agent · /onboard portal (complementary · probe only ≠ Connected)
 ...
 · dual_write OFF · book-demo OFF · Cloud Memory GA
 · residual PASS ≠ invent Connected · residual PASS ≠ live dogfood · PASS ≠ live APPLY
...
// Honesty: dual_write OFF · Cloud Memory GA · residual PASS ≠ invent Connected ·
...
func MeshAgentOnboardingNextE4Lane() string {
 ...
 · iomesh-memory-mcp · local-primary · dual_write OFF · book-demo OFF
 · Cloud Memory GA · residual PASS ≠ invent Connected · E10 Open
...
func MeshAgentOnboardingNextToolCallLane() string {
 ...
 · iomesh-memory-mcp · dual_write OFF · book-demo OFF
 · Cloud Memory GA · residual PASS ≠ invent Connected · E10 Open
 · tip ≠ invent forever-green product dogfood · residual PASS ≠ live dogfood · PASS ≠ live APPLY
 · soft offline ≠ invent Connected · session soft ≠ live dogfood · open boxes stay open
 · free eng s1578 · free-floor peer s1580+ mention only (do not rewrite free-floor)
...
// Honesty: residual PASS ≠ invent E10 closed · residual PASS ≠ invent Connected ·
// Cloud Memory GA · E10 stays Open · Cloud Memory GA ≠ close E10 ·
// Live APPLY still human · PASS ≠ live APPLY · dual_write OFF · book-demo OFF · Cloud Memory GA.
...
func MeshAgentOnboardingNextE10Lane() string {
  ...
  Path: residual-honest E10 Open reaffirm after OSS packaging continuum — soft offline residual-check only · never invent E10 closed / Connected / live APPLY as green
  Product: Platform residual honesty (optional anti-claims) · pin E10 Open · free eng s1586 · free-floor peer s1588+ mention only
  Steps:
    1. Pin E10 Open — E10 remains Open · residual PASS ≠ invent E10 closed · E10 Open
    2. Candidacy honesty — Cloud Memory GA · residual PASS ≠ invent Connected · Cloud Memory GA · E10 stays Open · Cloud Memory GA ≠ close E10
 ...
 · soft offline ≠ invent Connected · residual PASS ≠ live dogfood · session soft ≠ live dogfood · residual PASS ≠ invent Connected · residual PASS ≠ invent E10 closed
  Honesty:
 · E10 Open · E10 Open reaffirm · residual PASS ≠ invent E10 closed · residual PASS ≠ invent Connected
 · Cloud Memory GA · dual_write OFF · book-demo OFF
 · Cloud Memory GA · E10 stays Open · Cloud Memory GA ≠ close E10 · PASS ≠ live APPLY
...
//
// Constraints (do not overclaim): dual_write OFF · local memory · Cloud Memory GA ·
...
func MeshAgentOnboardingNextMarketingDemoLane() string {
 ...
 · dual_write OFF (default stays off)
    4. Start / attach iomesh-memory-mcp
 · go install github.com/iome-sh/iomesh-memory-mcp/cmd/iomesh-memory-mcp@v0.4.2 (published pin · kernel v1.5.12 · persist opt-in default OFF · extract needs this host · do not invent a newer pin)
 ...
 · ACK the miss via /dashboard ack · do not invent send/pay/ship
  ...
  Published pins (tip class · Cloud Memory GA · do not invent a newer pin): TUI v1.3.7 · MCP v0.4.2 · memory kernel v1.5.12
 ...
 · Optional: public Go / Python mesh SDKs for custom services outside the lean TUI (s1666) — not required for this demo
  What not to claim:
 · do not invent Connected product green
...
Locks: dual_write OFF · book-demo OFF · local memory · local-primary · iomesh-memory-mcp · mesh optional · never invent Connected · never invent install green / INSTALL_STORE APPLY · dual_write stays OFF (never invent primary ON) · TUI stays lean (no SDK dep) · not invent 1.0 · not invent PyPI green · Mode A sticky /memory digest --require-sources mesh,private · miss ≠ known until /dashboard ack · no send/pay/ship · palace path printed · air-gap fallback no fake Connected · pins TUI v1.3.7 · MCP v0.4.2 · memory kernel v1.5.12 · free eng s1590 · s1594 sales talk track · s1598 GTM claim-support · s1602 operator GTM boundary · s1606 GTM wave 6 · s1610 GTM wave 7 · s1614 GTM wave 8 · s1618 GTM wave 9 · s1666 free eng Python client SDK peer · free-floor peer s1592+ mention only`)
}
...
func MeshAgentOnboardingNextAgenticLane() string {
 ...
 · Complementary: /onboard portal mint/copy/probe @ https://console.iome.sh/settings/agent (probe only ≠ Connected)
 ...
 · dual_write OFF · book-demo OFF · residual PASS ≠ live dogfood · PASS ≠ live APPLY
...
func MeshAgentOnboardingNextThreePlanes() string {
 ...
 · mesh → local palace egress · iomesh memory pull · dual_write OFF
 · Ops Pack ≠ GPU fleet · pull ≠ freemium hosted palace · package load ≠ Ops Pack entitlement · package load ≠ Connected
 ...
 · dual_write OFF · book-demo OFF · residual PASS ≠ live dogfood · PASS ≠ live APPLY
 · open boxes stay open · human-gates still open (tip only)
...
Locks: dual_write OFF · book-demo OFF · residual PASS ≠ live dogfood · PASS ≠ live APPLY · open boxes stay open · mesh ≠ memory · never invent stream green / pull green / Connected / INSTALL_STORE APPLY · streams_not_probed · pull_not_probed · Ops Pack ≠ GPU fleet · pull ≠ freemium hosted palace · agent MCP cannot write installs · list_plan_not_connected · portal_hitl_still · dual_auth_candidacy_open · tool ship ≠ dual-auth live · never invent dual-auth live · catalog ≠ Connected · plan deep links = browser HITL only · template= ≠ install APPLY · rates ~$88/$119 optional · Cloud Memory GA · board/export evidence ≠ invent Connected · path_ready · residual_only · leave ON_SIGNAL unset · Knowledge Beta→GA cannot invent H1/H2 offline`)
}
...
// dual-auth live / human-gate green / freemium palace / Ops Pack GPU. Cloud Memory GA.
...
// Must not claim: invent Connected / INSTALL_STORE green · dual_write ON ·
...
func MeshAgentOnboardingNextSalesClaims() string {
  ...
  board for founders/sales operators — may claim vs must not claim only (never invent Connected / APPLY as success · Cloud Memory GA):
 ...
 · Cloud Memory GA · dual_write OFF · local-primary · Palace sunset · package load ≠ Connected
 · Salesforce = GA CRM · HubSpot + GTM suite Beta multi-tenant · guerrilla global-only
 · knowledge / analytical = Beta · knowledge/analytical stay Beta
 ...
 ·  invent dual_write as ON · freemium hosted palace · Ops Pack as GPU fleet · Ops Pack ≠ GPU fleet
...
// Landgrab READY / Connected · Cloud Memory GA / dual-auth live / human-gate green.
...
func MeshAgentOnboardingNextDemoReadiness() string {
 ...
 · dual_write OFF · never invent Connected · dual_auth_candidacy_open
 ...
 · never invent Connected · Cloud Memory GA / dual-auth live / human-gate green
...
Locks: dual_write OFF · book-demo OFF · Landgrab NOT READY · residual PASS ≠ live dogfood · PASS ≠ live APPLY · residual PASS ≠ logos met · open boxes stay open · never invent Connected / INSTALL_STORE APPLY · dual_auth_candidacy_open · tool ship ≠ dual-auth live · never invent dual-auth live · rates ~$88/$119 optional · Cloud Memory GA · leave ON_SIGNAL unset · Knowledge Beta→GA cannot invent H1/H2 offline · PASS ≠ invent human-gate green · board/export evidence ≠ invent Connected · founder-led walkthrough only when scheduled · operator runbook ≠ public /demo booking live · never invent book-demo as ON · never invent Landgrab as READY`)
}
...
// dual-auth live / human-gate green / book-demo ON / Landgrab READY. Cloud Memory GA.
...
//	6 Policy locks — dual_write OFF · Cloud Memory GA · leave ON_SIGNAL unset · rates ~$88/$119 optional
...
func MeshAgentOnboardingNextOperatorMatrix() string {
 ...
 · may claim / must not claim residual-honest · never invent Connected · Cloud Memory GA / dual-auth live
 ...
 · book-demo OFF · leave ON_SIGNAL unset · dual_write OFF · Cloud Memory GA
 · residual PASS ≠ invent Connected · PASS ≠ invent Connected · H1/H2 not launch gate
 · setup closeout residual ≠ invent APPLY (s1546) · Cloud Memory GA · E10 stays Open
 ...
 · dual_write OFF · leave ON_SIGNAL unset · book-demo OFF
 · rates ~$88 mesh / ~$119 Memory Ops Pack optional (commercial framing only · not product GA claim)
 · Cloud Memory GA · drafts only · no auto-send · residual PASS ≠ live dogfood · PASS ≠ live APPLY
...
// Never claims dogfood PASS live, Agent Plugins GA, or install Connected. Cloud Memory GA.
func MeshAgentOnboardingNextLaneStatus() string {
 ...
 · dual_write OFF · local-primary · package load ≠ Connected · ≠ freemium palace · Palace sunset
 · book-demo OFF · rates ~$88/$119 optional · mesh ≠ memory · mesh optional for pull
 · edge OSS tip (s1453+s1458+s1463+s1469+s1478+s1508): iomesh-memory-mcp product host · public product attach · go install · no GOPRIVATE · HTTP 8080/mcp or stdio · docker compose still valid · dual_write OFF · control-plane / broker private · Cloud Memory GA · public edge ≠ invent Connected · Cloud Memory GA · public OSS ≠ invent Connected · PASS ≠ invent full platform sidecar parity · offline dogfood tip ≠ invent live dogfood as green · E4 client attach (s1508) · Cloud Memory GA · residual PASS ≠ invent Connected · E10 Open · tip ≠ invent forever-green product dogfood
 ...
 · package load ≠ Ops Pack entitlement · package load ≠ Connected · Palace sunset
 ...
 · setup lifecycle P1–P7 closeout residual (s1542+s1558 · stage 4 of edge-user-journey) · offline static map · dual_write OFF
 ...
 · Cloud Memory GA · setup closeout ≠ invent Connected · offline static lane ≠ live dogfood
 ...
 · edge-user-journey first-run map (s1558 Wave B · 7 stages) · dual_write OFF · Cloud Memory GA
 · Signup → Download TUI → TUI auth/keys → Setup wizard → Connectors → Local store → Analyze
 · residual PASS ≠ invent Connected · portal HITL · no invent TUI portal SSO · host not auto · free eng s1558
 ...
 · first-run wizard residual (s1570 Wave C · guided residual map + soft dogfood) · dual_write OFF · Cloud Memory GA
 · residual PASS ≠ invent Connected · E10 Open · portal HITL when connect · agent MCP cannot write installs · catalog ≠ Connected
 ...
 · deeper tool-call residual (s1578 · stage 6/7 depth after E4 attach · soft offline residual) · dual_write OFF · Cloud Memory GA
 · memory_ingest_turn · memory_retrieve · memory_list · memory_facts_as_of path residual · Partial→client-attach-evidence
 · residual PASS ≠ invent Connected · E10 Open · tip ≠ invent forever-green product dogfood · free eng s1578 · free-floor peer s1580+ mention only
 ...
 · Agent/MCP mint/copy/probe @ https://console.iome.sh/settings/agent · connectors @ https://console.iome.sh/integrations
  ...
  sales/buyer claims: /onboard next sales (aliases claims|buyer|claim-matrix|sales-claims|buyer-claims) — may claim / must not claim residual-honest · three-planes grounded · never invent Connected · Cloud Memory GA (s1437)
  ...
  edge-user-journey first-run: /onboard next journey — 7 stages residual-honest · dual_write OFF · Cloud Memory GA · free eng s1558 (s1558 Wave B)
  ...
  tool-call soft offline: /onboard next tool-call dogfood (aliases soft|samples|offline|tool-call-soft) — soft offline ≠ invent Connected · ≠ live dogfood · free eng s1578 · free-floor peer s1580+ mention only
...
Locks: dual_write OFF · book-demo OFF · residual PASS ≠ live dogfood · PASS ≠ live APPLY · PASS ≠ invent human-gate green · open boxes stay open · never invent install green / Connected / INSTALL_STORE APPLY · catalog ≠ Connected · portal HITL · agent MCP cannot write installs · plugins dogfood ≠ invent Agent Plugins GA · session soft ≠ live dogfood · soft offline ≠ live dogfood · drafts only · no auto-send · package load ≠ Connected · rates ~$88/$119 optional · board/export evidence ≠ invent Connected · mesh = streaming org heartbeats · mesh ≠ memory · never invent stream green · streams_not_probed · not OTel/APM · pull_not_probed · pull ≠ freemium hosted palace · Ops Pack ≠ GPU fleet · never invent pull green · list_plan_not_connected · plan deep links = browser HITL only · template= ≠ install APPLY · dual_auth_candidacy_open · list_org_unavailable · tool ship ≠ dual-auth live · never invent dual-auth live · setup_not_probed · package wire ≠ Connected · repair apply ≠ invent Connected · dual_write never auto ON · E10 Open · Cloud Memory GA · setup closeout ≠ invent Connected · Knowledge Beta→GA cannot invent H1/H2 offline · leave ON_SIGNAL unset`, samplesState, dogfoodState, agenticSoft))
}
...
setup: path_ready · residual_only · setup_not_probed
 · setup lifecycle P1–P7 closeout residual (s1542) · offline static map · dual_write OFF
 · init → preflight → reload → portal HITL → pull → analyze → drift → repair plan/apply --yes
 · package wire ≠ Connected · repair apply ≠ invent Connected · dual_write never auto ON · E10 Open
 · Cloud Memory GA · setup closeout ≠ invent Connected · offline static lane ≠ live dogfood
...
- GTM checklist ≠ invent GTM agent GA · package load ≠ Connected
...
Locks: dual_write OFF · book-demo OFF · residual PASS ≠ live dogfood · PASS ≠ live APPLY · PASS ≠ invent human-gate green · open boxes stay open · never invent install green / Connected / INSTALL_STORE APPLY · catalog ≠ Connected · portal HITL · agent MCP cannot write installs · plugins dogfood ≠ invent Agent Plugins GA · session soft ≠ live dogfood · soft offline ≠ live dogfood · drafts only · no auto-send · package load ≠ Connected · board/export evidence ≠ invent Connected · rates ~$88/$119 optional · mesh = streaming org heartbeats · mesh ≠ memory · never invent stream green · streams_not_probed · pull_not_probed · pull ≠ freemium hosted palace · Ops Pack ≠ GPU fleet · never invent pull green · list_plan_not_connected · plan deep links = browser HITL only · template= ≠ install APPLY · dual_auth_candidacy_open · list_org_unavailable · tool ship ≠ dual-auth live · never invent dual-auth live · setup_not_probed · package wire ≠ Connected · repair apply ≠ invent Connected · dual_write never auto ON · E10 Open · Cloud Memory GA · setup closeout ≠ invent Connected · Knowledge Beta→GA cannot invent H1/H2 offline · leave ON_SIGNAL unset`, samplesState, dogfoodState, agenticSoft))
}
...
// unless ACL regresses · Cloud Memory GA · E10 stays Open · book-demo OFF.
// s1546: setup residual complete ≠ invent human-gate green / live APPLY / OAuth Connected / E10.
// Honesty locks: dual_write OFF · book-demo OFF · Cloud Memory GA ·
// residual PASS ≠ invent Connected · PASS ≠ invent Connected · agent MCP cannot write installs ·
...
func MeshAgentHumanGatesHonestyBoard() string {
 ...
 · dual_write OFF · Cloud Memory GA · E10 Open
 · residual PASS ≠ invent Connected · residual PASS ≠ invent Connected
 · Cloud Memory GA · E10 stays Open
 · optional E4 edge dogfood if local attach only
 ...
 · setup lifecycle residual complete ≠ invent Connected
 · Wave A–C continuum residual (journey · wizard · portal-hitl · e4 soft) ≠ invent human-gate green / live APPLY / Connected
 · agent MCP list/plan residual-honest · dual_write OFF · Cloud Memory GA
 · local memory / dual_write OFF do NOT invent Connected
 · residual PASS ≠ invent Connected · residual PASS ≠ invent Connected · PASS ≠ invent Connected · open policy boxes stay honest
