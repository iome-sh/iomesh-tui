...
// AionAgentOnboardingNextMemoryLane residual-honest memory local drill for /onboard next memory (s1377+s1453+s1458+s1463).
...
// alternate · offline dogfood tip ≠ invent live dogfood as green · residual PASS ≠ public flip · M3 after M2 · M4 later.
// May still attach residual aion-memory-mcp / local edge docker. Never invents Memory GA, freemium palace, dual_write ON,
// install Connected, live dogfood green, or public OSS flip complete.
func AionAgentOnboardingNextMemoryLane() string {
	return strings.TrimSpace(`aion onboard next memory lane (residual-honest · s1377+s1453+s1458+s1463 · no MCP dial):
  Path: local-primary Memory edge — TUI + Memory MCP + github.com/iome-sh/memory kernel + local palace — dual_write OFF
  Edge OSS (Option A · s1453+s1458+s1463 · residual-honest install + M2 lean attach + M3 edge dogfood):
    ...
    · residual: scaffold/M2 · M3 after M2 · tool parity may be lean vs platform residual · PASS ≠ invent full platform sidecar parity
    · offline dogfood tip ≠ invent live dogfood as green · residual PASS ≠ public flip · M4 public flip later deliberate
    · kernel = github.com/iome-sh/memory · OSS path ≠ invent public flip complete · cleanup-before-OSS residual · peer s1452 OSS bar mention only
  ...
  Steps:
    ...
    3. dual_write OFF · local-primary only · not Memory GA · Palace sunset · aion broker private · still private
    4. Optional: read_skill memory-advanced-agent (opt-in advanced · still dual_write OFF · not Memory GA)
    5. Optional mesh pull only: /onboard next memory-pull · Memory Ops Pack optional (~$119 pull/retain) · mesh optional for pull · dual_write OFF
    6. Operator pulse: /memory status · /onboard status · /onboard next operator (fail-open offline · never invent tool green)
  Honesty:
    ...
    · residual PASS ≠ public flip · M3 after M2 scaffold · M4 public flip later deliberate · still private
    · never invent install green / Connected / INSTALL_STORE APPLY
  ...
  Companion: /onboard next memory-pull · /onboard next operator · docs/architecture/memory-mcp.md Edge OSS Option A · M2 lean attach · M3 edge dogfood
  Back: /onboard next · /memory status · portal Agent/MCP https://console.iome.sh/settings/agent

Locks: dual_write OFF · book-demo OFF · not Memory GA · Palace sunset · residual PASS ≠ live dogfood · offline dogfood tip ≠ invent live dogfood as green · PASS ≠ live APPLY · PASS ≠ invent full platform sidecar parity · residual PASS ≠ public flip · scaffold/M2 lean residual · M3 after M2 · M4 later deliberate · still private · package load ≠ Memory GA · ≠ freemium palace · never invent install green / Connected / INSTALL_STORE APPLY · catalog ≠ Connected · portal HITL · agent MCP cannot write installs · rates ~$88/$119 optional · Memory Ops Pack optional · mesh ≠ memory · mesh optional for pull · iomesh-memory-mcp · aion broker private · OSS path ≠ invent public flip complete`)
}
...
func AionAgentOnboardingNextLaneStatus() string {
  ...
  memory: path_ready · residual_only
    ...
    · edge OSS tip (s1453+s1458+s1463): iomesh-memory-mcp product host · M2 lean attach when built · M3 edge dogfood (docker compose · 8080/mcp · healthz · stdio) · dual_write OFF · not Memory GA · aion broker private · OSS path ≠ invent public flip complete · PASS ≠ invent full platform sidecar parity · offline dogfood tip ≠ invent live dogfood as green · residual PASS ≠ public flip
    · drill: /onboard next memory (aliases mcp|palace)
