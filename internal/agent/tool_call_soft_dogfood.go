...
// Session-only: default tool_call_soft_not_run. Soft offline pass/fail ≠ invent Connected ·
...
const (
	...
	// ToolCallSoftPass is session soft offline tool-call residual PASS (≠ live dogfood · ≠ invent Connected).
...
// pass is residual soft offline only — never invents Connected / dual_write ON / live dogfood / E10 closed.
...
// Session soft marker ≠ live dogfood · ≠ invent Connected · residual PASS ≠ live dogfood.
...
// Never dials MCP · never starts host · never invents Connected · dual_write ON · E10 closed · forever-green product dogfood.
...
func RunDeeperToolCallSoftDogfood() string {
	...
	b.WriteString(" · residual PASS ≠ invent Connected · Cloud Memory GA · dual_write OFF · free eng s1578\n")
	b.WriteString("\n")
	if pass {
		...
		b.WriteString("  result: FAIL (soft offline residual only · ≠ invent red product · residual PASS ≠ invent Connected)\n")
	...
	b.WriteString("  companion: /onboard next e4 · /onboard next memory · /onboard next journey · /memory status · internal note removed from this public repository\n")
	b.WriteString("\n")
