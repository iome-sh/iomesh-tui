...
Lean MIT host **v0.4.2** does **not** advertise `memory_patterns_list` · `memory_anomalies_list` · `memory_timeline` · `memory_ingest_event` · `memory_trigger_compact` — those are **platform sidecar / MCP-first when present**, not the lean binary. Do not restamp E4 `tools=6` as live `tools=11`. PASS ≠ invent full platform sidecar parity.
...
| `memory_related` | Multi-hop lite related recall (`seed_entity` / `query` / `max_hops`; optional `prefer_shorter_hops` omit=true · s1135 + s1281 / mesh s1277) |
...
| `memory_supersede_entity` | A3 lite entity supersession (`entity` required; optional `as_of`; **HITL** · s1282 / mesh s640; **MCP-first** — no lean HTTP invent) |
| `memory_patterns_list` | Ops pulse Beta pattern list (shipped s1287 · **platform sidecar / MCP-first when present**; not on lean MIT host v0.4.2) |
| `memory_anomalies_list` | Ops pulse Beta anomaly list (shipped s1287 · **platform sidecar / MCP-first when present**; not on lean MIT host v0.4.2) |
| `memory_timeline` | Temporal timeline slice (s1296 slash: `/memory timeline`; **platform sidecar / MCP-first when present**) |
...
| `memory_ingest_event` | Ops/telemetry event ingest (s1301 slash: `/memory ingest-event`; s138 T1; **not** conversation turn; **platform sidecar / MCP-first when present**) |
...
| `memory_trigger_compact` | Mutating RecMem compaction advisory — **HITL wired** (s1311 slash: `/memory trigger-compact --i-confirm`; **platform sidecar / MCP-first when present**) |
| compact / other ops helpers | Residual ops helpers (Cloud Memory GA) |
...
| **3 related hop ranking (s1281)** | **done (opt-in)** | `prefer_shorter_hops` on HTTP + MCP `memory_related` (omit = kernel default **true**; `--legacy-sort` → false); mesh s1277 parity; hop ranking path-aware lite · multi-hop lite ≠ graph RAG |
| **3 ops digest (s1200)** | **done (opt-in)** | Sync `POST /v1|/v5/memory/ops_digest` + MCP `ops_digest_export` fallback; `/memory digest`; Cloud Memory GA · knowledge/analytical Beta |
| **3 facts-as-of (s1276)** | **done (opt-in · MCP-first)** | MCP `memory_facts_as_of` (mesh Beta K4 lite); `/memory facts-as-of`; bi-temporal lite · not full dual-clock Graphiti · no lean HTTP route today |
| **3 supersede HITL (s1282)** | **done (opt-in · MCP-first · HITL)** | MCP `memory_supersede_entity` (mesh A3 lite / s640); `/memory supersede --entity … --i-confirm`; mutating closes `valid_until`; not NLP contradiction · no lean HTTP invent |
| **3 patterns/anomalies (s1287)** | **done (opt-in · MCP ops pulse Beta)** | `/memory patterns|anomalies` + MCP `memory_patterns_list` / `memory_anomalies_list` when present; ops pulse Beta · not medical window engine · no lean HTTP invent |
...
| **3 timeline + compact-status (s1296)** | **done (opt-in · MCP-first · read-only compact)** | `/memory timeline` → MCP `memory_timeline`; `/memory compact-status` → MCP `memory_compact_status`; temporal timeline · filters before limit · Palace tier counts residual · dual_write OFF · mutating compact deferred to s1311 HITL · no lean HTTP invent |
| **3 semantic + ingest-event (s1301)** | **done (opt-in · MCP-first)** | `/memory semantic` → MCP `memory_search_semantic` (tier-4 semantic facts residual · empty ≠ invent); `/memory ingest-event` → MCP `memory_ingest_event` (s138 T1 temporal event telemetry · not conversation turn · never invent memory_id); dual_write OFF · no lean HTTP invent |
| **3 local-edge Docker attach docs (s1308)** | **done (docs only)** | Operator/agent docs for attaching **local-edge Docker** Memory MCP streamable HTTP (prefer product `iomesh-memory-mcp` compose; peer mesh **s1306** private compose residual + **s1307**); TUI `[[mcp.servers]]` URL attach · dual_write OFF · local-primary · hosted Palace sunset · **Cloud Memory GA** · Cloud Memory GA · docker edge ≠ invent Connected · no public registry tag claim |
| **3 edge OSS Option A install honesty (s1453)** | **done (docs + onboard memory lane)** | Residual-honest edge install story: product MCP host **`iomesh-memory-mcp`** · kernel `github.com/iome-sh/memory` · control-plane / broker **private** · dual_write OFF · Palace sunset · mesh optional for pull · **OSS path ≠ invent public flip complete** |
| **3 M2 lean host attach tip (s1458)** | **done (docs + onboard memory lane)** | Residual-honest **M2 lean edge host** attach when built from `github.com/iome-sh/iomesh-memory-mcp`: go run/build · streamable HTTP `http://127.0.0.1:8080/mcp` or stdio · dual_write OFF · control-plane / broker private · scaffold/M2 · tool parity may be lean vs platform residual · **PASS ≠ invent full platform sidecar parity** · keeps Option A honesty |
| **3 M3 edge dogfood tip (s1463)** | **done (docs + onboard memory lane)** | Residual-honest **M3 edge dogfood** TUI↔`iomesh-memory-mcp` path: build/run from product repo · `docker compose up --build` → image `iomesh-memory-mcp:local` · attach `http://127.0.0.1:8080/mcp` · healthz · stdio alternate · peer mcp `make edge-dogfood-gate` (mention only) · dual_write OFF · **offline dogfood tip ≠ invent live dogfood as green** · **PASS ≠ invent full platform sidecar parity** · residual PASS ≠ invent public flip (edge packs public as of s1478) · M3 after M2 · M4 later deliberate |
| **3 M4 public flip readiness tip (s1469)** | **done (docs + onboard memory lane)** | Residual-honest **M4 public flip readiness** tip history: order kernel `github.com/iome-sh/memory` **first** · then `iomesh-memory-mcp` · readiness docs/gates in those repos (mention only) · **readiness tip ≠ invent public flip complete** · residual PASS ≠ invent public flip · dual_write OFF · control-plane / broker private · M5 signing later after flip · **s1478 supersedes operator tip** (edge packs public) · keeps Option A + M2 + M3 honesty |
| **3 public product attach (s1478)** | **done (docs + onboard memory lane + product sample plugin)** | Residual-honest **post-public** product edge attach: both `github.com/iome-sh/memory` + `github.com/iome-sh/iomesh-memory-mcp` **PUBLIC** · `go install …@v0.4.2` / `go get …@v1.5.12` · **no GOPRIVATE** · HTTP `http://127.0.0.1:8080/mcp` or stdio · docker compose still valid · dual_write OFF · control-plane / broker **still private** · Cloud Memory GA · public edge ≠ invent Connected · Cloud Memory GA · public OSS ≠ invent Connected · control-plane / broker private · s1517 product-only in-tree sample (`@main` is tip of default branch, not a production pin) |
| **3 E4 MCP client attach dogfood (s1508)** | **done (docs + evidence stamp + onboard memory lane tip)** | Residual-honest **E4 full MCP client attach** dogfood: lean host HTTP → TUI `iomesh mcp --connect` **connected=1 · tools=6** (observed stamp) · dual_write OFF · local-primary · **Cloud Memory GA** · residual PASS ≠ invent Connected · Cloud Memory GA · control-plane / broker private · **E10 Open** · tip ≠ invent forever-green product dogfood · evidence internal note removed from this public repository |
| **3 memory edge usage demo (s1513)** | **done (docs only)** | Residual-honest **utilization/demo example**: signup (optional) → TUI MCP integrations list/plan + portal HITL → local memory install (kernel + `iomesh-memory-mcp`; **not fully automatic**) → attach → show `/memory` + `mcp --connect` usage · dual_write OFF · Cloud Memory GA · E10 Open · catalog ≠ Connected · control-plane / broker private · walkthrough [memory-edge-usage-demo.md](./memory-edge-usage-demo.md) |
| **3 trigger-compact HITL + advanced status (s1311)** | **done (opt-in · MCP-first · HITL)** | `/memory trigger-compact --i-confirm` → MCP `memory_trigger_compact` (RecMem advisory · mutating HITL · refuse without confirm); `/memory status` prints `MemoryStatusLine` + `MemoryAdvancedStatus` residual inventory (related · facts-as-of · supersede · timeline · compact-status · semantic · ingest-event · patterns · anomalies · digest · trigger-compact); dual_write OFF · not invent compaction green · no lean HTTP invent |
...
**Related (not Memory Palace):** agent connector setup slash `/integrations` (s1238/s1242/s1243) uses MCP `list_connector_catalog` / `plan_connector_setup` (mesh v178) + `get_webhook_signing_headers` (v30) with residual honesty — see [agent-integrations-setup.md](./agent-integrations-setup.md).
...
**s765 (Beta · completeness pin):** local edge stack **complete** after s761 first-class Ollama product Shipped — inventory (mesh pull egress ↔ TUI ↔ local memory MCP ↔ Ollama pin) locked by docs + unit tests (`TestDefaultModels_OllamaLocalEdgeCompletenessPin`: catalog name `ollama-llama3.2`, model id `llama3.2`, cost 0, caps `local`+`ollama`, priority after DeepSeek cascade / Premium). Completeness pin **does not** invent new catalog models and **does not** re-claim s761 product body. Peer mesh **s764** heartbeat-local-edge-path residual. Local AI ≠ platform GPU · dual_write OFF · Palace sunset · offline unit ≠ live Ollama green · catalog pin ≠ cascade default · not full mesh RBAC GA.

#### Local-primary LT honesty (s768 pin)

Pin so TUI surfaces agree with the **cost-max local-primary** charter after s761/s765 product bodies (this serial is docs/tests only — **does not** re-claim s761 Ollama product or s765 completeness inventory):
...
Peer mesh **s767** bi-heartbeats local-primary honesty continuum. Unit pin: `TestDefaultMemoryConfig` / config `Default().Memory.DualWrite == false` (s768). Offline unit ≠ live APPLY · Beta.

#### Naming honesty (s771 pin)

Docs **naming honesty** residual after mesh **s770** usage-rates honesty (this serial is docs/tests only — **does not** re-claim s761 Ollama product, s765 local-edge completeness, or s768 local-primary LT pin bodies):
...
Peer mesh **s770** usage-rates residual · prior TUI **s768** local-primary LT honesty. dual_write OFF · Palace sunset · offline unit ≠ live APPLY · Beta.

#### Buyer claim pin (s774)

OSS / buyer-claim boundary pin peer mesh **s773** cost-max claim suite expand (this serial is docs/tests only — **does not** re-claim s761 Ollama product, s765 local-edge completeness, s768 local-primary LT, or s771 naming honesty bodies):
...
| **Optional mesh pull** | Mesh pull into local palace — **optional** · **not first-run required**  | Cloud GPU / hosted Palace fleet as a first-run product · freemium hosted palace · pay-to-get-memory |
...
| **First-run ** | TUI OSS + `iomesh-memory-mcp` local-primary only · mesh optional | Mesh required for first-run · pay-to-get-memory invent |

Peer mesh **s773** cost-max claim suite expand · prior TUI **s771** naming · **s768** local-primary ·  mesh not first-run required. dual_write OFF · Palace sunset · local AI ≠ platform GPU · offline unit ≠ live APPLY · Beta. Unit pin peer: `TestDefault_DualWriteOff` / `TestDefaultMemoryConfig_DualWriteOff` (s768 body + s771/s774 comment peers).
...
**Private residual (optional):** private control-plane repo may still ship private local-edge compose (historical peer **s1306** / residual **s1307**) — not product naming, not required for public OSS attach, and not an in-tree TUI sample (s1517). dual_write OFF · control-plane / broker private.
...
| **Cloud Memory GA · docker edge ≠ invent Connected** | Cloud Memory is GA. Containerizing product Memory MCP does **not** invent platform GPU or a public registry tag |
| **image tag** | product local image `iomesh-memory-mcp:local` — do not claim public Hub/GHCR pull GA |
| **TUI tree** | Does **not** vendor private control-plane compose/Dockerfiles — operators use private control-plane repo for `docker compose` |
| **Peer residual** | peer mesh **s1306** compose body · peer mesh residual **s1307** · TUI docs **s1308** |
...
Residual-honest **edge Memory OSS install path** for operators (docs + `/onboard next memory` lane honesty — **does not** invent Memory GA, freemium palace, live dogfood green, Connected green, · Cloud Memory GA). **s1478:** both product edge repos are **public**. **s1508:** E4 full MCP client attach dogfood evidence stamp (local-primary · dual_write OFF · **Cloud Memory GA** · **E10 Open**).

| Layer | Role | Notes |
|-------|------|---------|
| **iomesh-tui** | Public MIT agent harness | Already public · local-primary client |
| **Memory MCP host** | Product name **`iomesh-memory-mcp`** only (public · go install / compose · s1517) | control-plane / broker private · no in-tree residual private Memory sample |
| **Kernel** | `github.com/iome-sh/memory` | **Public** (s1478) · `go get …@v1.5.12` · **no GOPRIVATE** (`@main` is tip of default branch, not a production pin) |
| **Local palace** | FS palace via MCP (`-palace-root` / docker edge) | Hosted Palace **sunset** · dual_write **OFF** |
| **control-plane / broker** | Private cloud control plane | **Stays private** · not OSS edge pack |
...
Optional mesh feed: credentials + platform for durable `iomesh memory pull` only. Mesh is **optional for pull only**, not required for local-primary Memory. **First-run :** OSS local-primary complete **without** mesh. Do not invent a priced add-on SKU or a mesh base rate here.
...
| **control-plane / broker private** | Cloud broker/CP stays private · not OSS edge pack |
...
| **healthz** | `curl -fsS http://127.0.0.1:8080/healthz` (probe only · ≠ invent tool green · Cloud Memory GA) |
...
| **post-s1478 public edge** | Host/kernel **public** (s1478) · control-plane / broker **still private** · control-plane / broker private · s1517 product-only in-tree sample |
...
TUI residual-honest **readiness tip history** (pre-flip). **s1478 supersedes operator tip** — edge packs are now public; readiness tip · Cloud Memory GA.
...
| **control-plane / broker private** | Cloud broker/CP stays private · not OSS edge pack · not flipped with edge pack |
...
| Surface | Residual honesty |
|---------|------------------|
| **Kernel public** | `go get github.com/iome-sh/memory@v1.5.12` · **no GOPRIVATE** (`@main` is tip of default branch, not a production pin) |
| **Host public** | `go install github.com/iome-sh/iomesh-memory-mcp/cmd/iomesh-memory-mcp@v0.4.2` (or clone) · **no GOPRIVATE** / PAT |
...
| **control-plane / broker private** | Cloud broker/CP **still private** · control-plane / broker private · s1517 product-only sample |
| **flip complete residual** | Flip complete residual · Cloud Memory GA · public OSS ≠ invent Connected · package load ≠ Connected |
| **Product sample plugin** | `examples/agent-plugins/iomesh-memory-mcp` (dogfood primary with hello-iome) |

```bash
# Public install — no GOPRIVATE / PAT (published pins)
...
- Streamable HTTP MCP (local edge docker when available — peer mesh s1306 compose; see [Local-edge Docker](#local-edge-docker-memory-mcp-s1308--peer-mesh-s1306) above)
- s1517: in-tree product sample is `examples/agent-plugins/iomesh-memory-mcp` only (residual private Memory sample removed)
- Product sample map: `examples/agent-plugins/iomesh-memory-mcp`
- Package load · Cloud Memory GA · docker edge ≠ invent Connected · Cloud Memory GA · public edge ≠ invent Connected · Cloud Memory GA · public OSS ≠ invent Connected · lean surface may lag platform residual · offline dogfood tip ≠ invent live dogfood as green
...
Residual-honest **full MCP client attach** dogfood for product tip TUI ↔ lean product host `iomesh-memory-mcp` over streamable HTTP. This is an **operator runbook + pinned evidence stamp** — Cloud Memory GA · not forever-green product dogfood.

**Cloud Memory GA (honesty frame):**
...
| **Cloud Memory GA** | Attach dogfood ≠ invent Connected |
| **Cloud Memory GA** | Local lean host ≠ multi-tenant hosted / freemium cloud palace |
| **control-plane / broker private** | Cloud broker/CP stays private · not OSS edge pack |
...
2. healthz OK — healthz may still return residual `not_memory_ga` · that field is not a product stamp · Cloud Memory GA · probe ≠ invent tool green:
...
| **Evidence note** | internal note removed from this public repository |

**Limits (s1453+s1458+s1463+s1469+s1478+s1508):**

| Claim | Truth |
|-------|--------|
| **dual_write** | **OFF** — optional mesh audit only · not primary palace |

| **Cloud Memory GA** | Cloud Memory is GA · local-primary · **PASS ≠ invent Connected** |
...
| **control-plane / broker private** | control-plane / broker stays private · not open-sourced with edge pack |
| **control-plane still private** | Cloud broker/CP private · not OSS edge pack · s1517 no in-tree residual private Memory sample |
...
Broker Role+Tenant ACL: `X-IOMesh-Role` requires Tenant, and Role+Tenant requires `filter_subject` under that tenant. A `dept.*` filter is not under an `org_*` tenant, so default (and documented) `dept.*` pull **omits** `X-IOMesh-Role` and the Role-gated `X-IOMesh-Tenant` bind — the no-Role create path that returns 201. Print `pull_role` is empty when omitted. Explicit `--filter org_*.events.>` still sends Role+Tenant. Create failures include the broker body and name the Role/Tenant/filter ACL. dual_write stays off.

**s705 (Beta):** PASS/summary text and `--json` always emit identity (`stream`, `consumer`, `filter_subject`, `pull_role`, `pull_allow_suffix`, `tenant` — empty string when unset), knobs (`dry_run`, `dual_write` report-only default false, `batch`, `max_wait_ms`, `once`), and counters including `last_error` (empty when none). Print DTO is `MemoryPullStatsPrint` (no omitempty on scraper keys). Does not invent pull success from identity fields alone. Peer create FormatConsumerInfo s696 + status/wait pull identity continuum; peer mesh s704. Fail-open empty role/tenant; dual_write default OFF; not full mesh RBAC GA.

**s717 (Beta):** process evidence always-emit on the same print DTO: `endpoint` / `org` / `workspace` (from `[iomesh]`; empty string honest when unset), `result` (`ok`|`err`), `exit_code` (0 success; 1 hard-fail non-cancel or soft-fail `errors>0 && ingested==0 && !dryRun`), `duration_ms` (wall-clock; 0 if not timed), `ack` knob. Early fail paths (mesh disabled / MCP missing) emit the DTO when possible. Process evidence ≠ invent pull success from identity alone. Peer mesh s716 residual after s705. Dual_write default OFF; not full mesh RBAC GA.

**s747 (Beta · completeness pin):** memory pull process evidence **complete** — identity (s705) + knobs/counters + process evidence (s717) locked by docs + unit tests (`MemoryPullStatsPrint` JSON always-emits `result` / `exit_code` / `endpoint` / `org` / `workspace` on empty and populated paths). Completeness pin **does not** invent new always-emit fields and **does not** re-claim s717 product body. Peer mesh s746 residual honesty. Process evidence ≠ invent pull success · dual_write OFF · not full mesh RBAC GA · offline unit ≠ live APPLY · empty honest.

**s675 (Beta):** optional `--role` / `[memory].pull_role` → `X-IOMesh-Role` and `--pull-allow-suffix` / `[memory].pull_allow_suffix` → `X-IOMesh-Pull-Allow-Suffix` on authenticated mesh requests (create/fetch/ack). Fail-open when empty (headers omitted). When the effective filter is `dept.*` and not under Tenant, Role and the Role-gated Tenant bind are omitted so consumer create can succeed (broker no-Role 201). Not full mesh IdP RBAC GA; dual_write remains optional audit (default OFF). Hosted Palace sunset — TUI path is local palace.

**s678 (Beta):** when `--filter` / `pull_filter` is empty, default `filter_subject` is role-aware (`DefaultMemoryPullFilterForRole`): empty role keeps s660; `agent`/`viewer` → `tenant.events.>`; `auditor` → `tenant.audit.>`; `operator`/`admin` → `tenant.>`; `custom` with exactly one allow-suffix token → `tenant.<suffix>.>`; custom multi/no suffix → empty (fail closed). Org-shaped tenants (`org_*`) remap to `dept.*` (or `dept.<department>`) before the role suffix so the default entitles `dept.*.events.*` wire subjects — not `org_<id>.events.>`. Explicit filter always wins. Peer mesh s678; not full mesh RBAC GA.

**s681 (Beta):** `iomesh mesh consumer create` also accepts `--role` / `[memory].pull_role` and `--pull-allow-suffix` / `[memory].pull_allow_suffix` (sets client `Config.Role` / `PullAllowSuffix` so create auth sends the same headers as memory pull). Empty `--filter` uses the same `DefaultMemoryPullFilterForRole` defaults (IOMesh tenant). A `dept.*` filter that is not under Tenant omits Role and the Role-gated Tenant bind (same default as memory pull). Fail-open without role; dual_write default OFF; peer mesh s680 continuum — not full mesh RBAC GA.

**s684 (Beta):** `iomesh mesh consumer fetch` (and ack/nack/delete) accept the same `--role` / `--pull-allow-suffix` flags and config fallbacks so broker fetch validates federated ACL headers (mesh s683 continuum). Fail-open without role; dual_write default OFF — not full mesh RBAC GA.

**s687 (Beta):** role=`memory` default filter → `tenant.memory.>` when tenant set (peer mesh s686 federated pull memory role; local-palace memory subjects only). Dogfood report always-emits `pull_role` / `pull_allow_suffix` from Client Config (empty when unset) for CI scrapers; dogfood CLI wires `[memory].pull_role` / `pull_allow_suffix` onto Client so the soft consumer probe sends headers + applies role-aware empty-filter defaults. Fail-open without role; dual_write default OFF; not full mesh RBAC GA.

**s690 (Beta):** `iomesh mesh status` always-emits `pull_role` / `pull_allow_suffix` (empty when unset) in text and JSON from the same Client Config path (`[memory].pull_role` / `pull_allow_suffix` wired onto Client like dogfood s687). CI scrapers can key stable identity without omitempty gaps. Fail-open without role; dual_write default OFF; not full mesh RBAC GA; peer mesh s689 residual gate continuum.

**s693 (Beta):** `iomesh mesh wait` always-emits `pull_role` / `pull_allow_suffix` (empty when unset) in text and JSON from the same Client Config path (`[memory].pull_role` / `pull_allow_suffix` wired onto Client like status s690). CI scrapers can key stable identity without omitempty gaps. Fail-open without role; dual_write default OFF; not full mesh RBAC GA; peer mesh s692 Ops Pack floors residual gate continuum.

**s696 (Beta):** `iomesh mesh consumer create` text (`FormatConsumerInfoWithAuth`) and JSON (`ConsumerInfoPrint`) always-emit `pull_role` / `pull_allow_suffix` (empty when unset) next to `filter_subject` from resolved create auth (s681). Wire `ConsumerInfo` decode stays free of auth fields. CI scrapers can key stable identity without omitempty gaps. Fail-open without role; dual_write default OFF; not full mesh RBAC GA; peer mesh s695 sales claim continuum.

**s705 (Beta):** `iomesh memory pull` always-emits full pull identity + knobs + counters in PASS/summary text and `--json` (`MemoryPullStatsPrint`) so scrapers are not limited to stderr start log. Empty role/tenant/filter honest; dual_write report-only default OFF; peer mesh s704 continuum — not full mesh RBAC GA.

**s717 (Beta):** same surface always-emits process evidence (`endpoint`/`org`/`workspace` empty honest; `result` ok|err; `exit_code` 0|1; `duration_ms`; `ack`) so CI scrapers record intended exit without shell `$?`. Soft-fail and hard-fail set `result=err`/`exit_code=1`; success sets `ok`/`0`. Process evidence ≠ invent pull success. Peer mesh s716 residual — dual_write default OFF; not full mesh RBAC GA.

**s747 (Beta · completeness pin):** process evidence surface complete (s705 + s717) — unit tests pin always-emit keys; no new DTO fields. Peer mesh s746 · process evidence ≠ invent pull success · dual_write OFF · not full mesh RBAC GA · offline unit ≠ live APPLY.

Env: `MEMORY_MCP_HTTP_ADDR` / `IOMESH_MEMORY_MCP_HTTP_ADDR`, path `MEMORY_MCP_HTTP_PATH` (default `/mcp`).
...
See internal note removed from this public repository for soft vs strict matrix. Unit coverage: `go test ./internal/iomesh` (httptest mock returns 200 on `/v1/streams/MEMORY_INGEST/publish`).

## Slash commands

Slash commands that call `memory_patterns_list` · `memory_anomalies_list` · `memory_timeline` · `memory_ingest_event` · `memory_trigger_compact` are **platform sidecar / MCP-first when present** — the public lean host (`iomesh-memory-mcp` v0.4.2) does **not** advertise those tools. Fail-open residual if missing. Do not restamp E4 `tools=6` as live `tools=11`.
...
| `/memory timeline\|tl […]` | Opt-in temporal timeline (s1296; MCP `memory_timeline`; **platform sidecar / MCP-first when present** — not on lean host v0.4.2) |
| `/memory compact-status\|compact` | Opt-in Palace tier counts residual (s1296; MCP `memory_compact_status`; read-only) |
| `/memory trigger-compact\|tcompact --i-confirm` | Opt-in HITL RecMem compact advisory (s1311; MCP `memory_trigger_compact`; **platform sidecar / MCP-first when present**; mutating) |
| `/memory semantic\|sem [query]` | Opt-in tier-4 semantic facts (s1301; MCP `memory_search_semantic`) |
| `/memory ingest-event\|event --subject … --content …` | Opt-in s138 T1 event telemetry (s1301; MCP `memory_ingest_event`; **platform sidecar / MCP-first when present**; not conversation turn) |
| `/memory patterns` / `/memory anomalies` | Opt-in MCP ops pulse Beta lists (shipped s1287; `memory_patterns_list` / `memory_anomalies_list`; **platform sidecar / MCP-first when present**) |
...
| `/memory extract [memory_id]` / `--id <id>` | Optional HITL structural extract after persist (MCP `memory_extract_facts`). `memory_id` required (fail closed). Tool missing on host → residual (do not invent facts). not NLP · dual_write OFF · never auto after ingest. |
| `/memory ingest-dir <path> [--dry-run] [--limit N] [--source-hint private] [--department id] [--scenario kit]` | Folder ingest into the private overlay (`#384` · V1.6 D2: 128 files × 64 KiB; allowlist `.md` `.txt` `.json` `.jsonl` `.csv` `.html`; PDF skip `export text first (no OCR)`). Workspace path jail. `source_hint=private` only (mesh/catalog/grant rejected). `--department` / `--scenario` tags; mint `local-overlay:{dept}` when department is set and no `--session-id`. CLI: `iomesh memory ingest-dir`. When `[iomesh].org` / `IOMESH_ORG` is set, mesh ingest (dual-write path) sends `X-IOMesh-Org`. Empty org fail-opens. dual_write default **OFF**. Catalog list ≠ consume. |
...
**Cite-both (#373 / #419 / #460, opt-in):** `--require-sources mesh,private` requires **both** mesh and private in the active receipt set, or prints an explicit miss. Classification uses receipt `source_hint`, `provenance.source_hint`, and tags (`source_hint:mesh`) — including nested palace `entry`/`content`/`metadata` and MCP `structuredContent` — export origin `palace_timeline` must not mask a stamped mesh turn. Sticky cite-both fetches up to the export cap, merges MCP palace receipts into the HTTP sidecar set, and pins an older mesh-stamped turn when it exists beside newer private RCA. If a required class is still missing and stamped turns of that class already exist on the **explicit tenant palace**, the newest stamp is pinned into the active receipt set with no day/week age gate. Other-org palaces and unstamped `palace_timeline` do not satisfy. If no stamped turn of that class exists, the miss names the newest-first receipt window (`limit` / `n` / `since` / `as_of`). Catalog-only / grant-only never satisfy cite-both (catalog list is not consume). Default `/memory digest` without the flag stays the existing ops pack. dual_write OFF · local palace on disk. Does not invent mesh.
...
### prefer_shorter_hops (s1281 · mesh s1277)
...
**Notes:** multi-hop lite opt-in · hop ranking path-aware lite · PreferShorterHops default true · **not** full graph RAG · **Cloud Memory GA** · dual_write default OFF · fail-open.
...
**Limits:** opt-in only (not auto-recall) · K4 bi-temporal **lite** · **not** full dual-clock Graphiti KG · **Cloud Memory GA** · dual_write OFF · empty facts ≠ invent memories · fail-open.
...
**Limits:** opt-in only · **HITL required** · A3 lite · closes `valid_until` · **not** NLP contradiction · **not** full dual-clock Graphiti · **Cloud Memory GA** · dual_write OFF · MCP-first (no lean HTTP invent) · fail-open · empty/zero count honest when wire says so.
...
| Notes | ops pulse **Beta** · **not** medical · **Cloud Memory GA** · dual_write OFF · **Cloud Memory GA** · fail-open |
...
**s1291:** on `AttachMCP`, runtime injects residual-honest `<memory-advanced>` system note from `MemoryAdvancedAgentGuidanceNote()` (mirror integrations s1251). Opt-in advanced locks only; does not invent Connected / silent supersede / auto multi-hop.
...
**Limits:** opt-in only · temporal timeline · Cloud Memory GA · filters before limit · compact status ≠ invent compaction green · not auto-compact product · dual_write OFF · MCP-first (no lean HTTP invent) · fail-open · empty entries honest.
...
**Limits:** HITL required · RecMem advisory ≠ invent compaction green · dual_write OFF · presence ≠ product green · MCP-first · fail-open · never invent triggered/cluster_size offline.
...
Residual-honest **next-step footers** after primary `/memory` surfaces — peer of onboard next-step (s1825) · integrations next-step (s1727) · setup continuum (setup continuum).
...
**Notes:** dual_write **OFF** · local-primary · package wire ≠ Connected · soft ≠ invent live dogfood · free eng **s1831**. Advanced slash surfaces (related · facts-as-of · timeline · …) keep their existing honesty footers and are **not** all re-wired in s1831 (fragmented residual).
...
- **s1308:** local-edge Docker MCP attach (peer mesh s1306) is customer-edge FS palace only — dual_write OFF · hosted Palace sunset · **Cloud Memory GA** · Cloud Memory GA · docker edge ≠ invent Connected · local image tag ≠ public registry claim.
- **s1453:** edge OSS Option A residual-honest install story — product host **`iomesh-memory-mcp`** · kernel `github.com/iome-sh/memory` · control-plane / broker **private** · dual_write OFF · Palace sunset · mesh optional for pull · **OSS path ≠ invent public flip complete**.
- **s1458:** M2 lean edge host attach tip — when built from `github.com/iome-sh/iomesh-memory-mcp`: go run/build · streamable HTTP `http://127.0.0.1:8080/mcp` or stdio · dual_write OFF · control-plane / broker private · scaffold/M2 · tool parity may be lean vs platform residual · **PASS ≠ invent full platform sidecar parity** · keeps Option A honesty.
- **s1463:** M3 edge dogfood tip — TUI↔`iomesh-memory-mcp` compose/HTTP/stdio path: `docker compose up --build` → image `iomesh-memory-mcp:local` · attach `http://127.0.0.1:8080/mcp` · healthz · stdio alternate · peer mcp `make edge-dogfood-gate` (mention only) · dual_write OFF · host/kernel **public as of s1478** · control-plane / broker private · **offline dogfood tip ≠ invent live dogfood as green** · **PASS ≠ invent full platform sidecar parity** · residual PASS ≠ invent public flip · M3 after M2 · M4 later deliberate · keeps Option A + M2 honesty.
- **s1469:** M4 public flip readiness tip — order kernel `github.com/iome-sh/memory` **first** · then `iomesh-memory-mcp` · readiness tip history · dual_write OFF · control-plane / broker private · **M5 signing later after flip** · **s1478 supersedes operator tip** (edge packs public) · keeps Option A + M2 + M3 honesty.
- **s1478:** public product attach continuum — both edge repos **public** · `go install github.com/iome-sh/iomesh-memory-mcp/cmd/iomesh-memory-mcp@v0.4.2` · `go get github.com/iome-sh/memory@v1.5.12` · **no GOPRIVATE** · HTTP `:8080/mcp` or stdio · docker compose still valid · dual_write OFF · control-plane / broker **still private** · Cloud Memory GA · public edge ≠ invent Connected · Cloud Memory GA · public OSS ≠ invent Connected · control-plane / broker private · s1517 product-only sample · product sample plugin `iomesh-memory-mcp` (`@main` is tip of default branch, not a production pin).
- **s1508:** E4 full MCP client attach dogfood — lean host HTTP → TUI `iomesh mcp --connect` observed **connected=1 · tools=6** (UTC `2026-08-09T06:23:34Z` · TUI tip `6b3958a…` · MCP tip `f46afe2…`) · dual_write OFF · local-primary · **Cloud Memory GA** · residual PASS ≠ invent Connected · Cloud Memory GA · control-plane / broker private · **E10 Open** · tip ≠ invent forever-green product dogfood · evidence internal note removed from this public repository.
- **s1513:** memory edge usage/demo example — residual-honest walkthrough signup (optional) → integrations list/plan + portal HITL → local kernel+MCP install (**not fully automatic**) → attach → show `/memory` usage · dual_write OFF · Cloud Memory GA · E10 Open · catalog ≠ Connected · control-plane / broker private · [memory-edge-usage-demo.md](./memory-edge-usage-demo.md).


## s1069 recall efficiency

Client-side short-TTL cache for sync `RetrieveMemory` (`[memory] recall_cache_ttl_ms`, default 3000; `0` disables). Keyed by tenant+session+query+limit+since/until. Fail-open process-local only — Cloud Memory GA. Snippet early-stop at `max_snippet_bytes`. Auto-recall events always emit retrieve latency (`Nms` / `Nms cache`).
- **s1517:** drop residual private Memory sample from TUI tree — product host/docs/onboard/`examples/agent-plugins` use **`iomesh-memory-mcp` only** · control-plane / broker private · dual_write OFF · no invent platform GA from rename.
- **s1525:** advanced Memory install ladder for TUI — optional ONNX on host maximizes semantic; Qdrant not required / not wired lean host search · dual_write OFF · [memory-advanced-install.md](./memory-advanced-install.md).
