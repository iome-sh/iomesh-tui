...
**s768 (Beta · honesty pin):** local-primary LT memory = local MCP palace + optional Ollama · dual_write default OFF · hosted Palace sunset · local memory is free MIT · **do not invent a priced SKU on the MIT local path** · peer mesh s767. See [memory-mcp.md Local-primary LT honesty](./memory-mcp.md#local-primary-lt-honesty-s768-pin).
