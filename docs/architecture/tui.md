...
- Knowledge / analytics stay **Beta**. Empty knowledge or analytics adds `knowledge Beta empty · analytics Beta empty · not GA`. Default EMPTY (no consume, including CLIENT with no messages) is heartbeat-only: no `analysis  ops 0` count row and no Ops Pack pull/insights/decision compose — that honesty lives in the footer. Preview/PULSE still show kind counts. `catalog ≠ Connected`. `dual_write OFF`. Cloud Memory GA. Not live APPLY. Kind from subject ≠ GA.
- **V1.6 D4:** entitled `dept.*` streams are domain output ports; `/dashboard` PULSE is the consume-clock (product live?); palace facts-as-of is the long temporal port after R4 `iomesh memory pull` (do not collapse with R2 facts-as-of); heartbeat kind chips filter consume (`ops`/`knowledge`/`analytics` via kind-from-subject) ≠ palace search (`/memory facts-as-of --department`); knowledge/analytics stay Beta empty until a signed event of that kind; pull fail-closed when `X-IOMesh-Org` set, local fail-open when org/tenant/role unset.
