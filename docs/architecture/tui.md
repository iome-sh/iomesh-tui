...
- **V1.6 D4:** entitled `dept.*` streams are domain output ports; `/dashboard` PULSE is the consume-clock (product live?); palace facts-as-of is the long temporal port after R4 `iomesh memory pull` (do not collapse with R2 facts-as-of); heartbeat kind chips filter consume by department stream (not palace search `/memory facts-as-of --department`); knowledge/analytics stay Beta empty until a signed event of that kind; pull fail-closed when `X-IOMesh-Org`/tenant/role set, local fail-open when unset.
