...
|-------|----------------|---------|
| TUI (`iomesh-tui`) | **v1.3.0** | SemVer 1.0+ harness · not hosted Memory GA |
...
1. **Setup story** — “On your laptop: install `iomesh` **v1.3.0**, set an LLM key or Ollama, `/setup init` with local-memory (dual_write stays **OFF**), attach `iomesh-memory-mcp` **v0.1.1+post-pin** (kernel **v1.5.8**).”
