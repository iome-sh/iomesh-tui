...
|-------|----------------|------|
| [iomesh-tui](https://github.com/iome-sh/iomesh-tui) | **v1.3.4** (`go install github.com/iome-sh/iomesh-tui/cmd/iomesh@v1.3.4` · [GitHub Release](https://github.com/iome-sh/iomesh-tui/releases/tag/v1.3.4)) | Agent harness / TUI |
...
```bash
go install github.com/iome-sh/iomesh-tui/cmd/iomesh@v1.3.4
...
> **Status:** public open-source **v1.3.x** (stable SemVer 1.0+; latest tag **v1.3.4**). Shipped: agent loop · subagents · full-screen TUI · **heartbeat dashboard** · permissions · ACP · skills · MCP client · **local memory attach** · sticky `/memory digest --require-sources mesh,private` (cite-both) · multi-model catalog (DeepSeek · Grok · Gemini · Vertex · Ollama). Optional mesh client when pointed at a broker you run or subscribe to. v1.3 is the MIT harness — not hosted Memory GA, not live HITL closed, not Knowledge/Analytics GA. dual_write OFF. catalog ≠ Connected.
...
Slash commands (TUI/REPL): `/model`, `/theme`, `/dashboard` (heartbeat live feed · aliases `/heartbeat` `/mesh-console`), `/permissions`, `/subagents`, `/setup`, `/onboard`, `/memory` (optional `/memory extract` HITL structural facts after persist · not NLP · dual_write OFF), `/integrations`, `/save`, `/sessions`, `/load`, `/cost`, `/help`, `/quit`.  
