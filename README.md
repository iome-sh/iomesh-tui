...
[![Release](https://img.shields.io/github/v/release/iome-sh/iomesh-tui)](https://github.com/iome-sh/iomesh-tui/releases/latest)
...
| Local memory | First-class attach to public `iomesh-memory-mcp` + kernel · `/setup` + `/memory` |
...
`/dashboard` is empty until consume (probe broker `/v1` if a mesh client is attached). `/dashboard preview` is the [iome.sh](https://iome.sh) landing MeshConsole **eval template** — not your workspace, not [console.iome.sh](https://console.iome.sh).
...
No GIF of a live tenant. Default `/dashboard` stays **empty** until consume (`ListStreams` + `ListStreamMessages` on the broker — **not** cookie-only `GET /v52`). `/dashboard preview` ticks the public seed every 2.6s (same cadence as the site).
