...
> **Status:** public open-source **foundation** (pre-1.0). Agent loop, subagents, full-screen TUI, permissions, ACP, skills, MCP, mesh dogfood. Not a multi-tenant remote sandbox — see [SECURITY.md](SECURITY.md).
...
export DEEPSEEK_API_KEY=…          # required for live LLM calls
# export XAI_API_KEY=…             # optional premium fallback
...
Optional: copy [`.env.example`](.env.example) for local env vars (iomesh reads the **process environment**; it does not auto-load `.env` files yet). Copy [`configs/config.example.toml`](configs/config.example.toml) to `~/.iomesh/config.toml` to customize models, mesh, MCP, UI theme, etc.
...
Override: `iomesh -m deepseek-v4-pro` or `/model` in the TUI. See [docs/architecture/llm-cascade.md](docs/architecture/llm-cascade.md).
...
Precedence: **CLI flags** → **environment** (`IOMESH_*`, `DEEPSEEK_API_KEY`, …) → **`~/.iomesh/config.toml`** → built-in defaults.
...
make build
make ci         # full local gate (matches CI spirit)
```
