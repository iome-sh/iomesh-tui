...
- **`mesh streams` list always-emit MAX_MSGS/MAX_AGE columns (s699)** — `FormatStreams` table always prints MAX_MSGS / MAX_AGE headers (blank when max_msgs/max_age_sec nil; numeric when set) for CI scrapers next to retention. Complements FormatStreamDetail always-emit. Beta · offline unit ≠ live APPLY · peer aion s698 cost-max residual suite continuum
