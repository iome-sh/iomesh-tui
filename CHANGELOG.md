...
- **Cite-both digest (#373):** `/memory digest --require-sources mesh,private` cites both mesh + private receipt `source_hint`s or prints an explicit miss. Catalog-only / grant-only never satisfies cite-both. dual_write OFF · not Memory GA · local palace on disk.
