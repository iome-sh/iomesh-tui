...
### Added
- **Stream catalog `org_id`:** `StreamInfo` decodes broker `org_id` on list/get. Empty `org_id` is shared persist (visible to every org) — never invent from `[iomesh].org`. `StreamInfoPrint` always emits `org_id` (empty honest). `FormatStreamDetail` and `FormatStreams` print it when present. dual_write OFF · catalog ≠ Connected · not Memory GA · local MIT.
- **Setup org residual + mesh pull/ingest `X-IOMesh-Org` (#386 / #387):** `/setup init mesh` and `iomesh setup init mesh` persist `[iomesh].org` / `IOMESH_ORG` (`--mesh-org` writes the field; empty writes a commented residual + fail-open honesty). Mesh pull/fetch/ack/create send `X-IOMesh-Org` when org is configured (CLI `--org` overlays config/env). Memory ingest and ingest-dir (#385) send the same header on the mesh dual-write path when org is set. Empty org still fail-opens locally — setup no longer silently omits the residual. Preflight always emits `org` (empty honest). Empty `/dashboard` stays honest (no fake pulse). dual_write **OFF** · not Memory GA · catalog ≠ Connected.
...
### Fixed
- **Preflight empty-org note:** `iomesh setup preflight` notes when mesh is enabled and `[iomesh].org` / `IOMESH_ORG` is empty. Hosted brokers isolate catalog/consume by `X-IOMesh-Org` and may 400 without it. Local/dev stays fail-open (not a hard fail). dual_write OFF · catalog ≠ Connected · not Memory GA · local MIT.
